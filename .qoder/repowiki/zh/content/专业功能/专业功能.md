# 专业功能

<cite>
**本文引用的文件**
- [README.md](file://README.md)
- [PromptAssistantPanel.tsx](file://client/src/components/PromptAssistantPanel.tsx)
- [MaskEditor.tsx](file://client/src/components/MaskEditor.tsx)
- [FaceSwapPhotoWall.tsx](file://client/src/components/FaceSwapPhotoWall.tsx)
- [ZITSidebar.tsx](file://client/src/components/ZITSidebar.tsx)
- [FavoriteFacesMenu.tsx](file://client/src/components/FavoriteFacesMenu.tsx)
- [useFavoriteFaces.ts](file://client/src/hooks/useFavoriteFaces.ts)
- [usePromptAssistantStore.ts](file://client/src/hooks/usePromptAssistantStore.ts)
- [useMaskStore.ts](file://client/src/hooks/useMaskStore.ts)
- [maskConfig.ts](file://client/src/config/maskConfig.ts)
- [systemPrompts.ts](file://client/src/components/prompt-assistant/systemPrompts.ts)
- [favorites.ts](file://server/src/routes/favorites.ts)
- [Pix2Real-提示词助手.json](file://ComfyUI_API/Pix2Real-提示词助手.json)
- [Pix2Real-自动识别.json](file://ComfyUI_API/Pix2Real-自动识别.json)
- [SystemPrompt.txt](file://docs/SystemPrompt.txt)
</cite>

## 更新摘要
**变更内容**
- 新增收藏功能到专业功能模块，增强面部交换的用户体验和工作流效率
- 收藏功能支持跨会话存储、去重机制和批量导入
- 新增收藏菜单、收藏状态管理和导入导出功能

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件聚焦 CorineKit Pix2Real 的专业功能，围绕提示词助手、蒙版编辑、快速出图（ZIT）、黑兽换脸、**收藏功能**等高级能力进行系统化说明。内容涵盖工作机制、数据流、交互流程、配置项与最佳实践，并提供面向高级用户的技巧与工作流优化建议。

## 项目结构
- 前端采用 Vite + React + TypeScript，通过 Zustand 管理状态，WebSocket 实时接收 ComfyUI 进度事件。
- 后端基于 Express，适配器模式封装不同工作流，统一加载 ComfyUI JSON 模板并动态注入参数。
- 专业功能主要分布在客户端组件与 ComfyUI 工作流模板中，分别对应提示词助手、蒙版编辑、ZIT 快出、黑兽换脸、**收藏功能**等。

```mermaid
graph TB
subgraph "前端"
UI["React 组件<br/>PromptAssistantPanel/MaskEditor/FaceSwapPhotoWall/ZITSidebar<br/>FavoriteFacesMenu"]
Store["Zustand 状态<br/>usePromptAssistantStore/useMaskStore/useFavoriteFaces"]
WS["WebSocket Hook<br/>useWebSocket"]
end
subgraph "后端"
API["Express 路由<br/>workflow.ts/session.ts/output.ts/favorites.ts"]
Adapters["适配器层<br/>Workflow0Adapter..Workflow9Adapter"]
Services["服务层<br/>comfyui.ts/sessionManager.ts"]
end
subgraph "ComfyUI"
Templates["工作流模板<br/>ComfyUI_API/*.json"]
end
UI --> Store
UI --> WS
UI --> API
API --> Adapters
Adapters --> Templates
Services --> Templates
WS --> API
```

**图表来源**
- [README.md:41-79](file://README.md#L41-L79)

**章节来源**
- [README.md:1-79](file://README.md#L1-L79)

## 核心组件
- 提示词助手：多模式面板，支持标签转换、创建变体、按需扩写、脑补后续、分镜生成、标签合成器；通过系统提示词驱动。
- 蒙版编辑：AI 自动识别与手动绘制双通道，支持多种叠加模式、撤销/重做、导出混合结果。
- ZIT 快出：基于文本生成图像的快速工作流，集成模型列表、LoRA 开关、采样器/调度器、批处理与草稿持久化。
- 黑兽换脸：专用 UI 区域，支持拖拽换脸、多选批量、任务队列与进度跟踪。
- **收藏功能**：跨会话存储的面容收藏系统，支持去重、批量导入、收藏管理与导入导出。

**章节来源**
- [PromptAssistantPanel.tsx:1-139](file://client/src/components/PromptAssistantPanel.tsx#L1-L139)
- [MaskEditor.tsx:1-375](file://client/src/components/MaskEditor.tsx#L1-L375)
- [ZITSidebar.tsx:1-635](file://client/src/components/ZITSidebar.tsx#L1-L635)
- [FaceSwapPhotoWall.tsx:1-1152](file://client/src/components/FaceSwapPhotoWall.tsx#L1-L1152)
- [FavoriteFacesMenu.tsx:1-211](file://client/src/components/FavoriteFacesMenu.tsx#L1-L211)

## 架构总览
提示词助手与 ZIT 快出均通过前端调用后端接口触发 ComfyUI 执行；蒙版编辑器在前端完成绘制与历史管理，必要时调用后端进行 AI 识别；黑兽换脸在专用区域完成拖拽与任务注册；**收藏功能通过前端状态管理和后端 API 完成跨会话存储**。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "前端组件"
participant WS as "WebSocket Hook"
participant API as "后端路由"
participant AD as "适配器/服务"
participant C as "ComfyUI"
U->>UI : 触发操作如"收藏/识别并填充"
UI->>API : 发送请求如 /api/favorites/faces 或 /api/workflow/mask/auto-recognize
API->>AD : 加载模板并注入参数
AD->>C : 推送工作流执行
C-->>WS : 实时进度事件
WS-->>UI : 更新界面状态
UI-->>U : 展示结果/提示
```

**图表来源**
- [MaskEditor.tsx:218-235](file://client/src/components/MaskEditor.tsx#L218-L235)
- [README.md:74-79](file://README.md#L74-L79)

## 详细组件分析

### 提示词助手
- 功能定位：将自然语言与标签双向互转，生成提示词变体、按需扩写、续拍与分镜脚本。
- 交互入口：ZIT 侧边栏的"提示词助理"按钮或面板内多标签页切换。
- 系统提示词：包含"自然语言→标签"、"标签→自然语言"、"创建变体"、"按需扩写"、"脑补后续"、"分镜生成"六类规则。
- 数据流：用户输入经系统提示词封装后提交到后端，后端以 ComfyUI 模板执行推理，返回结果回填到输入框。

```mermaid
sequenceDiagram
participant U as "用户"
participant Z as "ZITSidebar"
participant PA as "PromptAssistantPanel"
participant API as "后端 /api/workflow/prompt-assistant"
participant C as "ComfyUI 模板"
U->>Z : 点击"提示词助理"
Z->>PA : 打开面板并设置初始文本
U->>PA : 选择模式如"按需扩写"
PA->>API : POST {systemPrompt, userPrompt}
API->>C : 加载提示词助手模板并执行
C-->>API : 返回生成文本
API-->>PA : 回填到输入框
PA-->>U : 显示结果
```

**图表来源**
- [ZITSidebar.tsx:442-462](file://client/src/components/ZITSidebar.tsx#L442-L462)
- [PromptAssistantPanel.tsx:19-139](file://client/src/components/PromptAssistantPanel.tsx#L19-L139)
- [systemPrompts.ts:4-145](file://client/src/components/prompt-assistant/systemPrompts.ts#L4-L145)
- [Pix2Real-提示词助手.json:1-106](file://ComfyUI_API/Pix2Real-提示词助手.json#L1-L106)

**章节来源**
- [ZITSidebar.tsx:158-179](file://client/src/components/ZITSidebar.tsx#L158-L179)
- [PromptAssistantPanel.tsx:1-139](file://client/src/components/PromptAssistantPanel.tsx#L1-L139)
- [usePromptAssistantStore.ts:1-33](file://client/src/hooks/usePromptAssistantStore.ts#L1-L33)
- [systemPrompts.ts:4-145](file://client/src/components/prompt-assistant/systemPrompts.ts#L4-L145)
- [SystemPrompt.txt:1-146](file://docs/SystemPrompt.txt#L1-L146)

### 蒙版编辑器
- 模式与用途：
  - 模式 A：叠加模式，用于覆盖式绘制（如"解除装备"）。
  - 模式 B：A/B 混合模式，支持与输出结果叠加混合并导出。
- AI 自动识别：上传原图，调用后端接口，返回二值蒙版，前端应用到画布。
- 手动绘制：画笔尺寸、硬度、不透明度调节；撤销/重做；清空/反转；蒙版可见性切换。
- 导出混合：将原图与结果图按蒙版进行裁剪混合，保存到指定路径。

```mermaid
flowchart TD
Start(["打开蒙版编辑器"]) --> Mode{"模式选择"}
Mode --> |A 叠加| Overlay["切换叠加模式"]
Mode --> |B 混合| Blend["启用 A/B 混合"]
Overlay --> Tools["画笔工具<br/>尺寸/硬度/不透明度"]
Blend --> Tools
Tools --> Auto["识别并填充AI"]
Tools --> Manual["手动绘制"]
Auto --> History["撤销/重做/清空/反转"]
Manual --> History
History --> Export{"是否导出混合结果？"}
Export --> |否| End(["关闭"])
Export --> |是| Mix["原图×蒙版裁剪<br/>结果图×蒙版裁剪<br/>合并导出"]
Mix --> End
```

**图表来源**
- [MaskEditor.tsx:141-375](file://client/src/components/MaskEditor.tsx#L141-L375)
- [useMaskStore.ts:12-51](file://client/src/hooks/useMaskStore.ts#L12-L51)
- [maskConfig.ts:3-20](file://client/src/config/maskConfig.ts#L3-L20)
- [Pix2Real-自动识别.json:1-161](file://ComfyUI_API/Pix2Real-自动识别.json#L1-L161)

**章节来源**
- [MaskEditor.tsx:1-375](file://client/src/components/MaskEditor.tsx#L1-L375)
- [useMaskStore.ts:1-51](file://client/src/hooks/useMaskStore.ts#L1-L51)
- [maskConfig.ts:1-20](file://client/src/config/maskConfig.ts#L1-L20)

### ZIT 快出（快速文本生成图像）
- 模型与 LoRA：动态加载 UNet 与 LoRA 列表，支持启用/禁用 LoRA。
- 采样参数：步数、CFG、采样器、调度器、AuraFlow 偏移（Shift）。
- 提示词与快捷动作：支持自然语言↔标签互转、按需扩写等快捷操作。
- 批处理与草稿：支持批量生成与本地草稿持久化，自动生成命名。

```mermaid
sequenceDiagram
participant U as "用户"
participant Z as "ZITSidebar"
participant API as "后端 /api/workflow/9/execute"
participant AD as "Workflow9Adapter"
participant C as "ComfyUI"
U->>Z : 输入提示词/选择参数
Z->>Z : 读取草稿/模型列表
Z->>API : POST {clientId, prompt, steps, cfg, sampler, scheduler, ...}
API->>AD : 加载 ZIT 文生图模板
AD->>C : 推送工作流
C-->>Z : 返回 promptId
Z-->>U : 展示生成卡片/进度
```

**图表来源**
- [ZITSidebar.tsx:107-156](file://client/src/components/ZITSidebar.tsx#L107-L156)
- [ZITSidebar.tsx:158-179](file://client/src/components/ZITSidebar.tsx#L158-L179)

**章节来源**
- [ZITSidebar.tsx:1-635](file://client/src/components/ZITSidebar.tsx#L1-L635)

### 黑兽换脸
- 专用布局：左右分区（脸部参考/目标图），支持拖拽换脸与多选批量。
- 任务队列：检测目标图是否正在处理，避免并发冲突；支持多选批量换脸。
- 通信：通过 WebSocket 注册任务，实时更新进度。
- **收藏功能**：支持从收藏夹导入面容到脸部参考区，实现跨会话的面容复用。

```mermaid
sequenceDiagram
participant U as "用户"
participant FW as "FaceSwapPhotoWall"
participant API as "后端 /api/workflow/8/execute"
participant WS as "WebSocket"
participant C as "ComfyUI"
U->>FW : 将"脸部参考"拖到"目标图"
FW->>API : POST {targetImage, faceImage, clientId}
API->>C : 推送换脸工作流
C-->>WS : 进度事件
WS-->>FW : 更新任务状态
FW-->>U : 展示结果
```

**图表来源**
- [FaceSwapPhotoWall.tsx:257-282](file://client/src/components/FaceSwapPhotoWall.tsx#L257-L282)

**章节来源**
- [FaceSwapPhotoWall.tsx:1-1152](file://client/src/components/FaceSwapPhotoWall.tsx#L1-L1152)

### 收藏功能
- **功能定位**：跨会话存储和管理常用的面部参考，支持去重、批量导入和收藏管理。
- **核心技术**：基于 SHA-256 内容哈希进行去重，使用 multer 处理文件上传，JSON 文件存储元数据。
- **用户界面**：在脸部参考区域右上角提供收藏按钮，支持点击收藏/取消收藏；提供浮动菜单查看和管理收藏。
- **数据流**：前端计算文件哈希，后端存储文件和元数据，前端状态管理收藏列表和导入状态。

```mermaid
flowchart TD
Start(["用户点击收藏按钮"]) --> Hash["前端计算文件 SHA-256 哈希"]
Hash --> Check{"检查是否已收藏"}
Check --> |已收藏| Remove["调用后端 DELETE /api/favorites/faces/:id"]
Check --> |未收藏| Upload["调用后端 POST /api/favorites/faces"]
Upload --> Save["后端保存文件到 favorites/faces/<hash>.ext<br/>更新 metadata.json"]
Save --> Update["前端更新收藏列表状态"]
Remove --> Update
Update --> Menu["浮动菜单显示收藏列表"]
Menu --> Import{"用户从收藏导入？"}
Import --> |是| Fetch["前端获取收藏文件 URL"]
Fetch --> CheckDup["检查是否已在脸部参考区"]
CheckDup --> |已存在| Warn["提示已存在"]
CheckDup --> |不存在| Add["添加到脸部参考区并标记为已收藏"]
Add --> End(["完成"])
Warn --> End
```

**图表来源**
- [FaceSwapPhotoWall.tsx:329-351](file://client/src/components/FaceSwapPhotoWall.tsx#L329-L351)
- [useFavoriteFaces.ts:69-97](file://client/src/hooks/useFavoriteFaces.ts#L69-L97)
- [favorites.ts:62-89](file://server/src/routes/favorites.ts#L62-L89)

**章节来源**
- [FaceSwapPhotoWall.tsx:180-243](file://client/src/components/FaceSwapPhotoWall.tsx#L180-L243)
- [FaceSwapPhotoWall.tsx:329-385](file://client/src/components/FaceSwapPhotoWall.tsx#L329-L385)
- [FavoriteFacesMenu.tsx:1-211](file://client/src/components/FavoriteFacesMenu.tsx#L1-L211)
- [useFavoriteFaces.ts:1-110](file://client/src/hooks/useFavoriteFaces.ts#L1-L110)
- [favorites.ts:1-114](file://server/src/routes/favorites.ts#L1-L114)

## 依赖关系分析
- 组件耦合：
  - PromptAssistantPanel 依赖 usePromptAssistantStore 与 Mode* 子面板。
  - MaskEditor 依赖 useMaskStore、maskConfig 与 MaskCanvas。
  - ZITSidebar 依赖 useWorkflowStore、useWebSocket、systemPrompts。
  - FaceSwapPhotoWall 依赖 useWorkflowStore、useWebSocket、**useFavoriteFaces**。
  - **FavoriteFacesMenu 依赖 useFavoriteFaces**。
- 外部依赖：
  - ComfyUI 模板通过适配器加载并注入参数。
  - WebSocket 单例连接确保多组件共享进度事件。
  - **multer 用于文件上传处理**。
  - **crypto 用于 SHA-256 哈希计算**。

```mermaid
graph LR
PA["PromptAssistantPanel"] --> UPS["usePromptAssistantStore"]
ME["MaskEditor"] --> UM["useMaskStore"]
ME --> MC["maskConfig"]
ZS["ZITSidebar"] --> UWS["useWebSocket"]
ZS --> SP["systemPrompts"]
FS["FaceSwapPhotoWall"] --> UWS
FS --> UFF["useFavoriteFaces"]
FFM["FavoriteFacesMenu"] --> UFF
UFF --> Crypto["crypto.sha256"]
```

**图表来源**
- [PromptAssistantPanel.tsx:1-139](file://client/src/components/PromptAssistantPanel.tsx#L1-L139)
- [MaskEditor.tsx:1-375](file://client/src/components/MaskEditor.tsx#L1-L375)
- [ZITSidebar.tsx:1-635](file://client/src/components/ZITSidebar.tsx#L1-L635)
- [FaceSwapPhotoWall.tsx:1-1152](file://client/src/components/FaceSwapPhotoWall.tsx#L1-L1152)
- [FavoriteFacesMenu.tsx:1-211](file://client/src/components/FavoriteFacesMenu.tsx#L1-L211)

**章节来源**
- [README.md:74-79](file://README.md#L74-L79)

## 性能考量
- VRAM 释放：后端提供释放内存接口，可在长时间批量处理后清理显存。
- 采样参数：合理设置步数与 CFG，可平衡质量与速度；AuraFlow 偏移仅在特定采样器生效。
- 蒙版识别：自动识别前可先缩放图像以提升分割效率；适当调整扩展与模糊参数减少噪点。
- 批处理：ZIT 支持批量生成，建议根据显存与队列长度分批执行，避免阻塞。
- **收藏性能**：前端使用惰性计算缓存 SHA-256 哈希，避免重复计算；后端使用内存存储元数据，提高访问速度。

**章节来源**
- [README.md:13](file://README.md#L13)
- [ZITSidebar.tsx:528-557](file://client/src/components/ZITSidebar.tsx#L528-L557)
- [Pix2Real-自动识别.json:1-161](file://ComfyUI_API/Pix2Real-自动识别.json#L1-L161)
- [useFavoriteFaces.ts:99-108](file://client/src/hooks/useFavoriteFaces.ts#L99-L108)

## 故障排查指南
- 提示词助手无响应
  - 检查后端接口 /api/workflow/prompt-assistant 是否可达。
  - 确认系统提示词已正确传入，且 ComfyUI 模板加载成功。
- 蒙版识别失败
  - 确认上传图像格式与尺寸；检查 /api/workflow/mask/auto-recognize 返回状态码与错误信息。
  - 调整 SAM3 分割阈值与扩展/模糊参数。
- ZIT 生成卡住
  - 查看 WebSocket 是否正常接收进度；检查采样器/调度器与模型配置是否匹配。
  - 若显存不足，尝试降低分辨率或步数，或使用释放内存功能。
- 黑兽换脸报错
  - 确保目标图未处于处理中；检查 clientId 与文件上传是否成功。
- **收藏功能问题**
  - **收藏失败**：检查网络连接和后端 /api/favorites/faces 接口状态；确认文件大小限制和格式支持。
  - **取消收藏失败**：检查文件删除权限和 metadata.json 写入权限。
  - **导入失败**：确认收藏文件 URL 可访问；检查跨域设置；验证文件格式兼容性。
  - **收藏列表为空**：检查 favorites/faces 目录是否存在和可写；确认 metadata.json 格式正确。

**章节来源**
- [MaskEditor.tsx:218-235](file://client/src/components/MaskEditor.tsx#L218-L235)
- [ZITSidebar.tsx:137-156](file://client/src/components/ZITSidebar.tsx#L137-L156)
- [FaceSwapPhotoWall.tsx:266-282](file://client/src/components/FaceSwapPhotoWall.tsx#L266-L282)
- [favorites.ts:62-89](file://server/src/routes/favorites.ts#L62-L89)
- [useFavoriteFaces.ts:69-97](file://client/src/hooks/useFavoriteFaces.ts#L69-L97)

## 结论
本项目通过模块化的前端组件与适配器模式的后端架构，实现了提示词助手、蒙版编辑、ZIT 快出、黑兽换脸与**收藏功能**等专业功能。**新增的收藏功能通过跨会话存储和去重机制，显著提升了面部交换的工作流效率**。建议在实际使用中结合系统提示词规则、合理的采样参数与蒙版策略，配合批处理、显存管理以及收藏功能，以获得稳定高效的产出体验。

## 附录

### 使用场景与最佳实践
- 提示词助手
  - 场景：需要将中文描述精确映射为英文标签，或把标签还原为可读性强的中文描述。
  - 最佳实践：先用"自然语言→标签"提取关键视觉元素，再用"按需扩写"细化细节，最后用"创建变体"探索不同风格。
- 蒙版编辑
  - 场景：真人精修、解除装备等需要局部重绘或替换。
  - 最佳实践：优先使用"识别并填充"，再用"反转"修正误检区域；导出混合时保留原图与结果图以便比对。
- ZIT 快出
  - 场景：快速验证提示词效果与风格，批量生成对比素材。
  - 最佳实践：启用 LoRA 时注意模型兼容性；使用草稿功能保存常用参数组合。
- 黑兽换脸
  - 场景：多人物场景下的批量换脸与一致性保持。
  - 最佳实践：多选目标图时谨慎操作，避免并发冲突；换脸前确保目标图清晰且无遮挡。
- **收藏功能**
  - **场景**：需要跨会话复用常用的面部参考，如特定演员、角色或训练素材。
  - **最佳实践**：定期整理收藏列表，删除重复或过期的面容；利用收藏导入功能快速批量添加；注意文件大小和格式兼容性。

### 高级技巧与工作流优化
- 提示词工程：利用系统提示词的严格规则，先拆解再重构，逐步提升细节密度。
- 蒙版策略：先粗后细，AI 识别作为初稿，手动微调提升边界精度；批量处理时统一画笔参数。
- 采样器选择：不同采样器/调度器对风格影响显著，建议建立参数对照表。
- 任务编排：ZIT 与换脸可并行执行，但需关注队列与显存占用，必要时分时段运行。
- **收藏管理**：建立标准化的收藏命名规范；定期备份 favorites 目录；利用收藏导入功能优化批量工作流。