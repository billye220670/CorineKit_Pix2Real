...
## 更新摘要
**变更内容**
- **动态文本框高度调整功能**：为正向提示词和负向提示词新增自动高度调整功能，使用React的useLayoutEffect钩子实现内容驱动的高度变化
- **最小高度保持机制**：正向提示词最小高度保持80像素（4行），负向提示词通过首次渲染测量默认高度作为最小值
- **优化的视觉呈现效果**：动态高度调整提升了文本编辑体验，避免了固定高度带来的不便
...
- **动态文本框高度调整功能**：为正向提示词和负向提示词提供自动高度调整，使用React的useLayoutEffect钩子实现内容驱动的高度变化
...
- **标准化采样算法设置的间距配置**，统一各工作流侧边栏的间距一致性
- **动态文本框高度调整功能**：正向提示词最小高度保持80像素（4行），负向提示词通过首次渲染测量默认高度作为最小值
...
#### 正向提示词动态高度调整
**新增** Text2ImgSidebar 新增了智能的正向提示词文本框高度调整功能：

- **useLayoutEffect钩子**：使用React的useLayoutEffect钩子在布局阶段后执行高度调整逻辑
- **内容驱动调整**：根据文本内容的实际滚动高度动态调整textarea的高度
- **最小高度保持**：正向提示词最小高度保持80像素（4行），确保基本编辑空间
- **自动收缩**：当文本内容减少时，textarea会自动收缩到合适的高度
- **性能优化**：通过useLayoutEffect确保在DOM更新后立即执行高度计算，避免闪烁

#### 负向提示词动态高度调整
**新增** 负向提示词同样具备智能的高度调整功能：

- **首次渲染测量**：在组件首次渲染时测量默认高度，作为负向提示词的最小高度基准
- **动态高度计算**：使用negMinHeight状态变量存储测量到的最小高度
- **内容自适应**：根据negativePrompt状态的变化动态调整textarea高度
- **高度限制机制**：确保负向提示词不会小于首次渲染时测量到的默认高度
- **状态同步**：通过useLayoutEffect钩子确保高度调整与内容变化保持同步

#### 高度调整实现原理
- **ref引用管理**：使用textareaRef和negTextareaRef分别管理正向和负向提示词的DOM引用
- **scrollHeight计算**：通过element.scrollHeight获取内容的实际高度
- **高度设置逻辑**：先设置为'auto'清除之前的高度，再设置为Math.max(minHeight, scrollHeight) + 'px'
- **性能考虑**：useLayoutEffect在布局阶段执行，避免重排重绘的性能问题

#### 视觉呈现效果优化
- **平滑过渡动画**：高度变化时提供平滑的过渡效果，提升用户体验
- **内容溢出处理**：通过overflow: 'hidden'确保内容不会溢出textarea边界
- **盒模型一致性**：使用boxSizing: 'border-box'确保padding和border包含在总高度内
- **透明度反馈**：在快速转换进行时提供透明度变化，提示用户当前状态

**章节来源**
- [Text2ImgSidebar.tsx:138-164](file://client/src/components/Text2ImgSidebar.tsx#L138-L164)
- [Text2ImgSidebar.tsx:669-703](file://client/src/components/Text2ImgSidebar.tsx#L669-L703)
- [Text2ImgSidebar.tsx:827-853](file://client/src/components/Text2ImgSidebar.tsx#L827-L853)
...
- **Text2ImgSidebar 特有的动态文本框高度调整功能**，提升文本编辑体验。
...
- **动态文本框高度调整使用useLayoutEffect**，在布局阶段执行，避免重排重绘。
- **高度调整逻辑使用scrollHeight计算**，确保准确的高度测量。
- **最小高度保持机制**：正向提示词保持80px，负向提示词保持首次渲染测量的高度。
...
- **动态文本框高度调整失效**：检查useLayoutEffect钩子是否正确执行，确认textareaRef引用是否有效。
- **正向提示词高度异常**：检查最小高度保持逻辑，确认80px的最小高度设置正确。
- **负向提示词高度异常**：检查negMinHeight状态管理，确认首次渲染高度测量逻辑正确。
...
- **提示词编写技巧**
  ...
  - **动态高度调整优势**：正向提示词会根据内容自动调整高度，最小保持80px，无需手动调整。
- **参数调优建议**
  ...
  - **动态高度调整**：正向提示词会自动适应内容长度，负向提示词保持首次测量的高度。
- **批量生成**
...
- **动态文本框高度调整使用指南**
  - **正向提示词**：最小高度保持80px，内容增加时自动扩展，减少手动调整
  - **负向提示词**：首次渲染后测量默认高度，作为最小高度基准，确保编辑空间
  - **性能优化**：使用useLayoutEffect在布局阶段执行，避免重排重绘
  - **视觉反馈**：高度变化时提供平滑过渡，提升用户体验
...
NegativePrompt --> HeightAdjustment["动态文本框高度调整"]
HeightAdjustment --> RatioSelection["选择方面比例"]
...
- [Text2ImgSidebar.tsx:484-487](file://client/src/components/Text2ImgSidebar.tsx#L484-L487)
- [Text2ImgSidebar.tsx:514-516](file://client/src/components/Text2ImgSidebar.tsx#L514-L516)
- [Text2ImgSidebar.tsx:143-164](file://client/src/components/Text2ImgSidebar.tsx#L143-L164)
...
- **正向提示词**：最小高度保持80px，内容增加时自动扩展，减少手动调整
- **负向提示词**：首次渲染后测量默认高度，作为最小高度基准，确保编辑空间
- **useLayoutEffect钩子**：在布局阶段执行高度调整，避免重排重绘
- **scrollHeight计算**：通过element.scrollHeight获取内容的实际高度
- **性能优化**：先设置为'auto'清除之前的高度，再设置为Math.max(minHeight, scrollHeight) + 'px'
- **视觉反馈**：高度变化时提供平滑过渡，提升用户体验
- **状态管理**：通过negMinHeight状态变量存储测量到的最小高度
- **兼容性**：与现有的提示词快速转换功能完美集成
