# 负向提示词支持

<cite>
**本文档引用的文件**
- [README.md](file://README.md)
- [Text2ImgSidebar.tsx](file://client/src/components/Text2ImgSidebar.tsx)
- [ModeTagAssemble.tsx](file://client/src/components/prompt-assistant/ModeTagAssemble.tsx)
- [systemPrompts.ts](file://client/src/components/prompt-assistant/systemPrompts.ts)
- [usePromptAssistantStore.ts](file://client/src/hooks/usePromptAssistantStore.ts)
- [PromptAssistantPanel.tsx](file://client/src/components/PromptAssistantPanel.tsx)
- [tagData.json](file://client/src/data/tagData.json)
- [Pix2Real-提示词助手.json](file://ComfyUI_API/Pix2Real-提示词助手.json)
- [Pix2Real-提示词反推Flo.json](file://ComfyUI_API/Pix2Real-提示词反推Flo.json)
- [Pix2Real-提示词反推Q3.json](file://ComfyUI_API/Pix2Real-提示词反推Q3.json)
- [Pix2Real-提示词反推WD14.json](file://ComfyUI_API/Pix2Real-提示词反推WD14.json)
</cite>

## 更新摘要
**变更内容**
- 完全实现了负向提示词系统，支持与正向提示词相同的快速操作功能
- 新增了完整的负向提示词输入界面和交互功能
- 实现了负向提示词的三种快速操作：按需扩写、标签转换、自然语言转换
- 集成了提示词助手面板的负向提示词处理能力
- 支持标签合成器与负向提示词的协同工作
- **新增** 负向提示词输入组件的动态高度调整功能，使用useLayoutEffect钩子实现内容驱动的高度变化，确保用户能够看到完整提示词内容而无需滚动

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

本项目是一个基于ComfyUI的本地Web界面，用于批量图像/视频处理。项目的核心功能包括5种内置工作流、批量处理、实时进度更新、VRAM释放等特性。本文档重点关注负向提示词支持功能，这是一个重要的图像生成辅助功能，允许用户通过多种方式管理和优化负向提示词。

**更新** 负向提示词系统现已完全实现，支持与正向提示词相同的完整功能集，包括快速操作、提示词助手集成、标签合成器支持、多种提示词反推技术和**动态高度调整功能**。

负向提示词支持功能通过以下方式实现：
- 直接输入负向提示词的文本框
- 与提示词助手的深度集成
- 与标签合成器的协同工作
- 多种提示词反推技术的支持
- 与正向提示词相同的快速操作功能
- **新增** 动态高度调整功能，确保用户能够看到完整提示词内容而无需滚动

## 项目结构

项目采用前后端分离的架构设计：

```mermaid
graph TB
subgraph "前端客户端 (client)"
A[React + TypeScript 应用]
B[组件系统]
C[状态管理]
D[类型定义]
end
subgraph "后端服务 (server)"
E[Express 服务器]
F[适配器模式]
G[路由处理]
H[服务层]
end
subgraph "工作流模板 (ComfyUI_API)"
I[JSON模板文件]
J[提示词助手]
K[提示词反推]
L[标签合成器]
end
A --> E
B --> A
C --> A
D --> A
E --> F
F --> I
G --> H
H --> I
```

**图表来源**
- [README.md:41-62](file://README.md#L41-L62)

**章节来源**
- [README.md:1-79](file://README.md#L1-L79)

## 核心组件

### 负向提示词输入组件

负向提示词功能通过Text2ImgSidebar组件实现，提供了一个专门的输入区域，其结构与正向提示词完全一致：

```mermaid
classDiagram
class Text2ImgSidebar {
+string negativePrompt
+boolean negPromptFocused
+handleNegQuickAction()
+renderNegQuickActions()
}
class NegPromptInput {
+string placeholder
+onChange()
+onFocus()
+onBlur()
+renderQuickActions()
+dynamicHeightAdjustment()
}
class QuickActionButtons {
+handleDetailer()
+handleAssistant()
+handleReversePrompt()
}
Text2ImgSidebar --> NegPromptInput : "包含"
NegPromptInput --> QuickActionButtons : "包含"
```

**更新** 负向提示词输入组件与正向提示词组件具有完全相同的结构和功能实现，包括快速操作按钮、状态管理和事件处理，以及新增的动态高度调整功能。

**图表来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

### 动态高度调整功能

**新增** 负向提示词输入组件现在支持动态高度调整，使用useLayoutEffect钩子实现内容驱动的高度变化：

```mermaid
sequenceDiagram
participant User as 用户
participant Component as 负向提示词组件
participant Hook as useLayoutEffect
participant DOM as DOM节点
User->>Component : 输入或删除文本
Component->>Hook : 触发状态更新
Hook->>DOM : 计算内容高度
DOM->>DOM : 设置最小高度限制
DOM-->>User : 自动调整输入框高度
```

**更新** 动态高度调整功能通过以下步骤实现：
1. 首次渲染后测量输入框的默认高度并设置为最小高度
2. 当内容发生变化时，计算新的滚动高度
3. 将新高度与最小高度比较，取较大值
4. 应用到输入框样式，确保内容完整显示

**图表来源**
- [Text2ImgSidebar.tsx:151-164](file://client/src/components/Text2ImgSidebar.tsx#L151-L164)

### 提示词助手集成

负向提示词与提示词助手的深度集成，支持所有提示词助手模式：

```mermaid
sequenceDiagram
participant User as 用户
participant UI as 负向提示词界面
participant Store as 状态管理
participant Assistant as 提示词助手
participant API as 后端API
User->>UI : 输入负向提示词
UI->>Store : openPanel({initialText})
Store->>Assistant : 设置activeMode='convert'
Assistant->>API : POST /api/workflow/prompt-assistant
API-->>Assistant : 返回处理后的提示词
Assistant-->>UI : 更新负向提示词内容
UI-->>User : 显示结果
```

**更新** 提示词助手现在支持负向提示词的完整处理流程，包括标签转换、按需扩写、变体生成等所有模式。

**图表来源**
- [Text2ImgSidebar.tsx:760-772](file://client/src/components/Text2ImgSidebar.tsx#L760-L772)
- [usePromptAssistantStore.ts:15-32](file://client/src/hooks/usePromptAssistantStore.ts#L15-L32)

### 标签合成器支持

负向提示词与标签合成器的协同工作，提供结构化的标签管理系统：

```mermaid
flowchart TD
A[用户输入负向提示词] --> B[标签合成器模式]
B --> C[选择相关标签类别]
C --> D[多选/单选标签]
D --> E[自动生成负向提示词]
E --> F[优化和调整]
F --> G[应用到图像生成]
H[手动编辑] --> I[直接修改文本]
I --> G
```

**更新** 标签合成器现在专门为负向提示词提供标签管理功能，支持与正向提示词相同的标签组织结构。

**图表来源**
- [ModeTagAssemble.tsx:108-126](file://client/src/components/prompt-assistant/ModeTagAssemble.tsx#L108-L126)

**章节来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)
- [ModeTagAssemble.tsx:108-126](file://client/src/components/prompt-assistant/ModeTagAssemble.tsx#L108-L126)

## 架构概览

负向提示词支持的整体架构，现已与正向提示词系统完全对称：

```mermaid
graph TB
subgraph "用户界面层"
A[负向提示词输入框]
B[快速操作按钮]
C[提示词助手面板]
D[标签合成器]
end
subgraph "状态管理层"
E[Zustand状态管理]
F[本地存储持久化]
G[会话状态控制]
end
subgraph "业务逻辑层"
H[提示词处理函数]
I[快速操作处理器]
J[系统提示词管理]
K[标签数据管理]
end
subgraph "数据访问层"
L[后端API接口]
M[ComfyUI工作流]
N[模型加载器]
end
A --> E
B --> E
C --> E
D --> E
E --> F
E --> G
E --> H
E --> I
H --> J
I --> K
J --> L
K --> L
L --> M
M --> N
```

**更新** 架构现在实现了正负向提示词的完全对称设计，包括相同的状态管理模式、相同的业务逻辑处理、相同的后端接口调用，以及新增的动态高度调整功能。

**图表来源**
- [usePromptAssistantStore.ts:15-32](file://client/src/hooks/usePromptAssistantStore.ts#L15-L32)
- [PromptAssistantPanel.tsx:19-138](file://client/src/components/PromptAssistantPanel.tsx#L19-L138)

## 详细组件分析

### 负向提示词输入组件

#### 组件结构分析

```mermaid
classDiagram
class NegPromptContainer {
+boolean negPromptFocused
+boolean negQuickActionLoading
+string negativePrompt
+renderNegQuickActions()
+dynamicHeightAdjustment()
}
class NegPromptTextarea {
+string placeholder
+onChange()
+onFocus()
+onBlur()
+readOnly
+rows
+minHeight
}
class QuickActionButtons {
+handleDetailer()
+handleAssistant()
+handleReversePrompt()
+handleNaturalToTags()
}
class ActionButton {
+boolean disabled
+onClick()
+renderIcon()
}
NegPromptContainer --> NegPromptTextarea : "包含"
NegPromptContainer --> QuickActionButtons : "包含"
QuickActionButtons --> ActionButton : "包含"
```

**更新** 负向提示词输入组件与正向提示词组件在结构上完全一致，包括状态管理、事件处理、UI渲染逻辑，以及新增的动态高度调整功能。

**图表来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

#### 快速操作功能

负向提示词支持三种与正向提示词完全相同的快速操作：

1. **按需扩写 (Detailer)**: 使用系统提示词扩展负向提示词的细节
2. **提示词助手 (Assistant)**: 打开完整的提示词助手面板进行复杂处理
3. **提示词反推 (Reverse Prompt)**: 从图像内容反推出负向提示词

**更新** 负向提示词的快速操作功能与正向提示词完全对称，包括相同的处理逻辑、相同的系统提示词调用和相同的UI反馈机制。

**章节来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

### 动态高度调整实现

**新增** 负向提示词输入组件的动态高度调整功能通过以下实现：

```mermaid
flowchart TD
A[组件初始化] --> B{首次渲染?}
B --> |是| C[测量默认高度]
C --> D[设置negMinHeight]
D --> E[等待内容变化]
B --> |否| E
E --> F{内容变化?}
F --> |是| G[计算新高度]
G --> H{新高度 > 最小高度?}
H --> |是| I[应用新高度]
H --> |否| J[保持最小高度]
I --> K[更新样式]
J --> K
F --> |否| K
K --> L[用户继续输入]
L --> E
```

**更新** 动态高度调整功能的关键实现点：
1. **最小高度设置**: 首次渲染后测量输入框的默认高度作为最小高度
2. **内容驱动调整**: 当内容发生变化时，计算新的滚动高度
3. **高度限制**: 确保高度不会小于初始的最小高度
4. **平滑过渡**: 使用CSS过渡效果确保高度变化的视觉流畅性

**图表来源**
- [Text2ImgSidebar.tsx:151-164](file://client/src/components/Text2ImgSidebar.tsx#L151-L164)

#### 状态管理机制

```mermaid
stateDiagram-v2
[*] --> 初始化
初始化 --> 测量默认高度
测量默认高度 --> 等待内容变化
等待内容变化 --> 内容变化检测
内容变化检测 --> 计算新高度
计算新高度 --> 应用高度限制
应用高度限制 --> 更新样式
更新样式 --> 等待内容变化
```

**更新** 状态管理机制确保了动态高度调整的准确性和性能优化，避免不必要的重渲染。

**章节来源**
- [Text2ImgSidebar.tsx:151-164](file://client/src/components/Text2ImgSidebar.tsx#L151-L164)

### 提示词助手系统

#### 系统提示词管理

提示词助手提供了六种不同的处理模式，现在完全支持负向提示词：

```mermaid
graph LR
subgraph "提示词助手模式"
A[标签转换]
B[创建变体]
C[按需扩写]
D[脑补后续]
E[分镜生成]
F[标签合成器]
end
subgraph "系统提示词"
G[NaturalToTags]
H[TagsToNatural]
I[Variations]
J[Detailer]
K[NextScene]
L[Storyboarder]
end
A --> G
B --> H
C --> I
D --> J
E --> K
F --> L
```

**更新** 提示词助手现在为负向提示词提供完整的处理能力，所有模式都支持负向提示词的转换和优化。

**图表来源**
- [systemPrompts.ts:4-144](file://client/src/components/prompt-assistant/systemPrompts.ts#L4-L144)

#### 模式转换流程

```mermaid
sequenceDiagram
participant User as 用户
participant Panel as 提示词助手面板
participant Store as 状态管理
participant API as 后端服务
participant Model as AI模型
User->>Panel : 选择标签转换模式
Panel->>Store : setMode('convert')
Store->>Panel : 更新activeMode
Panel->>API : POST /api/workflow/prompt-assistant
API->>Model : 调用LLM处理
Model-->>API : 返回处理结果
API-->>Panel : 返回转换后的文本
Panel-->>User : 显示结果
```

**更新** 模式转换流程现在完全支持负向提示词的处理，包括与正向提示词相同的处理逻辑和结果返回机制。

**图表来源**
- [PromptAssistantPanel.tsx:19-138](file://client/src/components/PromptAssistantPanel.tsx#L19-L138)
- [systemPrompts.ts:4-25](file://client/src/components/prompt-assistant/systemPrompts.ts#L4-L25)

**章节来源**
- [systemPrompts.ts:4-144](file://client/src/components/prompt-assistant/systemPrompts.ts#L4-L144)
- [PromptAssistantPanel.tsx:19-138](file://client/src/components/PromptAssistantPanel.tsx#L19-L138)

### 标签合成器集成

#### 标签数据结构

标签合成器为负向提示词提供了结构化的标签管理系统，与正向提示词共享相同的数据结构：

```mermaid
erDiagram
TAG_DATA {
string id PK
string label
}
CATEGORY {
string id PK
string label
boolean multiSelect
}
SUBCATEGORY {
string id PK
string label
boolean multiSelect
}
TAG {
string id PK
string label
string value
}
TAG_DATA ||--o{ CATEGORY : "包含"
CATEGORY ||--o{ SUBCATEGORY : "包含"
SUBCATEGORY ||--o{ TAG : "包含"
```

**更新** 标签数据结构现在专门为负向提示词提供标签管理，支持与正向提示词相同的标签组织和选择机制。

**图表来源**
- [tagData.json:1-174](file://client/src/data/tagData.json#L1-L174)

#### 负向提示词生成流程

```mermaid
flowchart TD
A[用户选择标签] --> B[标签值收集]
B --> C[标签值合并]
C --> D[生成负向提示词]
D --> E[优化格式]
E --> F[应用到负向提示词框]
G[手动编辑] --> H[直接修改文本]
H --> F
I[导入标签数据] --> J[加载预设标签]
J --> B
```

**更新** 负向提示词生成流程现在与正向提示词流程完全对称，包括相同的标签选择逻辑、格式化处理和结果应用机制。

**图表来源**
- [ModeTagAssemble.tsx:108-126](file://client/src/components/prompt-assistant/ModeTagAssemble.tsx#L108-L126)

**章节来源**
- [tagData.json:1-174](file://client/src/data/tagData.json#L1-L174)
- [ModeTagAssemble.tsx:108-126](file://client/src/components/prompt-assistant/ModeTagAssemble.tsx#L108-L126)

### 提示词反推技术

#### 多模型支持

项目集成了三种不同的提示词反推技术，现在完全支持负向提示词：

```mermaid
graph TB
subgraph "提示词反推模型"
A[Florence-2]
B[Qwen3-VL]
C[WD14Tagger]
end
subgraph "输入处理"
D[图像加载]
E[参数配置]
F[模型选择]
end
subgraph "输出处理"
G[文本提取]
H[格式化]
I[结果保存]
end
D --> E
E --> F
F --> G
G --> H
H --> I
A --> G
B --> G
C --> G
```

**更新** 提示词反推技术现在为负向提示词提供完整的图像理解能力，支持与正向提示词相同的多模型反推功能。

#### 工作流配置

每种反推模型都有特定的工作流配置，现在支持负向提示词的处理：

| 模型 | 文件名 | 主要用途 | 关键参数 |
|------|--------|----------|----------|
| Florence-2 | Pix2Real-提示词反推Flo.json | 详细描述生成 | more_detailed_caption |
| Qwen3-VL | Pix2Real-提示词反推Q3.json | 中文描述和提示词 | preset_prompt: 极致详细 |
| WD14Tagger | Pix2Real-提示词反推WD14.json | 标签反推 | threshold: 0.35 |

**更新** 所有反推模型现在都支持负向提示词的处理，包括相同的参数配置和结果格式化。

**章节来源**
- [Pix2Real-提示词反推Flo.json:1-77](file://ComfyUI_API/Pix2Real-提示词反推Flo.json#L1-L77)
- [Pix2Real-提示词反推Q3.json:1-106](file://ComfyUI_API/Pix2Real-提示词反推Q3.json#L1-L106)
- [Pix2Real-提示词反推WD14.json:1-58](file://ComfyUI_API/Pix2Real-提示词反推WD14.json#L1-L58)

## 依赖关系分析

### 组件间依赖

```mermaid
graph TB
subgraph "UI组件层"
A[Text2ImgSidebar]
B[PromptAssistantPanel]
C[ModeTagAssemble]
end
subgraph "状态管理层"
D[usePromptAssistantStore]
E[useWorkflowStore]
end
subgraph "数据层"
F[tagData.json]
G[systemPrompts.ts]
end
subgraph "外部依赖"
H[ComfyUI API]
I[LLM模型]
J[图像处理模型]
end
A --> D
B --> D
C --> F
D --> G
A --> H
B --> I
C --> J
```

**更新** 依赖关系现在实现了正负向提示词的完全对称，包括相同的状态管理、相同的系统提示词和相同的外部依赖调用，以及新增的动态高度调整功能。

**图表来源**
- [usePromptAssistantStore.ts:15-32](file://client/src/hooks/usePromptAssistantStore.ts#L15-L32)
- [PromptAssistantPanel.tsx:19-138](file://client/src/components/PromptAssistantPanel.tsx#L19-L138)

### 数据流分析

```mermaid
sequenceDiagram
participant UI as 用户界面
participant Store as 状态管理
participant Backend as 后端服务
participant Model as AI模型
participant ComfyUI as ComfyUI引擎
UI->>Store : 用户输入负向提示词
Store->>UI : 更新界面状态
UI->>Backend : 发送处理请求
Backend->>Model : 调用AI模型
Model-->>Backend : 返回处理结果
Backend->>ComfyUI : 执行工作流
ComfyUI-->>Backend : 返回执行结果
Backend-->>UI : 更新最终状态
```

**更新** 数据流现在完全支持负向提示词的处理，包括与正向提示词相同的处理流程和结果返回机制。

**图表来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

**章节来源**
- [usePromptAssistantStore.ts:15-32](file://client/src/hooks/usePromptAssistantStore.ts#L15-L32)
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

## 性能考虑

### 内存管理

负向提示词功能在设计时充分考虑了性能优化，与正向提示词系统完全一致：

1. **懒加载机制**: 提示词助手面板仅在需要时加载
2. **状态缓存**: 使用localStorage持久化用户偏好设置
3. **条件渲染**: 根据焦点状态动态显示快速操作按钮
4. **防抖处理**: 避免频繁的状态更新触发
5. ****新增** 动态高度调整优化**: 使用useLayoutEffect确保高度计算的准确性，避免不必要的重排重绘

### 并发处理

```mermaid
flowchart TD
A[用户输入负向提示词] --> B{是否需要处理?}
B --> |是| C[异步处理请求]
B --> |否| D[直接应用]
C --> E[显示加载状态]
C --> F[调用后端API]
F --> G[等待响应]
G --> H[更新UI状态]
H --> I[隐藏加载状态]
D --> I
```

**更新** 并发处理机制现在完全支持负向提示词的处理，包括与正向提示词相同的异步处理和状态管理，以及新增的动态高度调整优化。

## 故障排除指南

### 常见问题及解决方案

1. **负向提示词不生效**
   - 检查提示词格式是否正确
   - 确认未被其他工作流覆盖
   - 验证ComfyUI连接状态

2. **提示词助手无响应**
   - 检查后端服务是否正常运行
   - 验证AI模型加载状态
   - 查看浏览器控制台错误信息

3. **标签合成器异常**
   - 确认tagData.json格式正确
   - 检查本地存储权限
   - 重新加载页面尝试

4. **负向提示词快速操作失效**
   - 检查负向提示词输入框是否有内容
   - 确认网络连接正常
   - 验证系统提示词配置

5. ****新增** 负向提示词高度调整问题**
   - 检查浏览器兼容性
   - 确认useLayoutEffect钩子正常工作
   - 验证CSS样式未被覆盖
   - 检查最小高度设置是否正确

**更新** 新增了负向提示词专用的故障排除指南，包括动态高度调整功能的问题排查和解决方案。

**章节来源**
- [Text2ImgSidebar.tsx:637-772](file://client/src/components/Text2ImgSidebar.tsx#L637-L772)

## 结论

负向提示词支持功能通过精心设计的架构实现了以下目标：

1. **用户体验优化**: 提供与正向提示词完全相同的负向提示词输入界面和快速操作按钮，以及新增的动态高度调整功能
2. **功能完整性**: 集成提示词助手、标签合成器和多种提示词反推技术
3. **性能保证**: 采用状态管理和懒加载机制确保流畅体验，以及优化的动态高度调整算法
4. **扩展性强**: 模块化设计便于未来功能扩展和维护
5. **对称性设计**: 负向提示词系统与正向提示词系统完全对称，提供一致的使用体验

**更新** 负向提示词系统现已完全实现，支持与正向提示词相同的完整功能集，包括快速操作、提示词助手集成、标签合成器支持、多模型提示词反推技术和**动态高度调整功能**。该系统不仅提升了图像生成的质量和效率，还为用户提供了更加灵活和强大的提示词管理工具，实现了正负向提示词的完全对称支持。

通过负向提示词的支持，用户可以更精确地控制生成结果，避免不需要的元素出现在最终图像中，同时享受与正向提示词相同的便捷操作、强大功能和**流畅的动态高度调整体验**。