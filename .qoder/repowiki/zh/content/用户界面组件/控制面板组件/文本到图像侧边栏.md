...
## 更新摘要
**变更内容**
- 移除了 ModelSelect 组件中的触发词显示和复制功能
- 新增了基于 PromptContextMenu 的右键插入机制，支持从 LoRA 触发词和标签库中插入内容
- 增强了 LoRA 模型使用状态的可视化提醒，在启用的 LoRA 模型旁显示警告三角形图标
- 更新了提示词输入框的右键菜单功能，提供更便捷的触发词插入方式
- 优化了提示词管理流程，从传统的触发词显示改为基于上下文菜单的智能插入
...
## 核心组件
- **Text2ImgSidebar**：负责收集用户输入的文本提示、尺寸、采样参数，构建请求并触发工作流执行，同时管理本地草稿与批量生成。现已采用统一的卡片布局系统，提供一致的视觉体验，并集成了新的 PromptContextMenu 提供右键插入功能。
...
- **启用状态指示**：在每个启用的 LoRA 模型行显示开关按钮，直观显示启用状态
...
## 结论
Text2ImgSidebar 将参数配置、提示词管理与 ComfyUI 工作流无缝衔接，提供了直观的文本到图像生成体验。通过本地草稿、批量生成与提示词快速转换等特性，显著提升了创作效率。**统一的卡片布局系统**进一步增强了用户体验，包括统一的颜色变量、动画效果和间距系统，提供了更加一致和现代化的视觉体验。**新增的 PromptContextMenu 右键插入机制**为用户提供了更便捷的触发词管理方式，支持从 LoRA 触发词和标签库中智能插入内容。**增强的 LoRA 使用状态可视化提醒**帮助用户更好地管理模型使用情况，避免遗漏重要的触发词。与其他工作流侧边栏相比，Tab 7 的纯文本输入设计使其专注于高质量文本生成，避免了外部图片干扰，适合需要精确提示词控制的场景。
...
- **统一卡片布局系统使用技巧**
  - 利用统一的卡片样式，所有参数区域具有一致的视觉体验。
  - 使用颜色变量系统，确保在明暗主题间的一致性。
  - 享受动画效果带来的交互反馈，提升使用体验。
  - 通过统一的间距系统，获得更加舒适的视觉感受。
- **右键插入功能使用指南**
  - 在提示词输入框右键打开菜单，选择合适的触发词或标签。
  - 支持从 LoRA 模型的触发词库中选择，或从标签库的多级分类中选择。
  - 插入后自动处理逗号分隔，保持提示词格式的正确性。
