...
## 更新摘要
**变更内容**
- 新增阶段化进度显示功能：ProgressOverlay 组件新增 stage、stepIndex、stepTotal 属性
- ImageCard 组件集成新的进度信息，支持工作流阶段显示
- 服务器端支持权重化阶段进度计算，提供更精确的进度反馈
- 增强了 Tab7 和 Tab9 工作流的进度显示能力
...
**更新** 组件现已支持全新的阶段化进度显示功能，通过 ProgressOverlay 组件的 stage、stepIndex、stepTotal 属性，为用户提供更精确的工作流阶段信息。服务器端实现了权重化的进度计算，能够准确反映每个处理阶段的完成情况。
...
**图表来源**
- [ImageCard.tsx:1-1429](file://client/src/components/ImageCard.tsx#L1-L1429)
...
**更新** 现在 Tab9 工作流支持阶段化进度显示，提供更精确的工作流阶段信息。

## 架构概览

ImageCard 采用分层架构设计，通过 Zustand 实现高效的状态管理，并集成了新的阶段化进度系统：
...
**更新** 最重要的更新是新增了对阶段化进度显示的全面支持。通过 ProgressOverlay 组件的 stage、stepIndex、stepTotal 属性，组件现在能够为用户提供精确的工作流阶段信息，包括当前处理阶段、步骤索引和总步骤数。服务器端实现了权重化的进度计算，能够准确反映每个处理阶段的完成情况。
