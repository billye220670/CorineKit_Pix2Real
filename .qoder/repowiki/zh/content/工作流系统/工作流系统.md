...
- 新增工作流 7 的参考图像管理功能，支持ControlNet深度和姿态处理
- 增强PRO模板模式支持，新增POST/GET/DELETE /api/workflow/7/ref-image端点
- 补充深度参考（depthStrength）和姿态参考（poseStrength）参数配置
- 更新工作流适配器映射表以包含新增 LoRA 支持和参考图像功能
...
**更新** 新增多槽位 LoRA 支持，允许在单个工作流中同时使用多个 LoRA 模型，并通过动态节点重连和条件路由实现智能的 LoRA 链接功能。新增工作流 7 的参考图像管理功能，支持ControlNet深度和姿态处理，增强PRO模板模式的图像参考能力。
...
RefHandler["参考图像处理器<br/>ControlNet深度/姿态"]
end
subgraph "ComfyUI"
...
- [client/src/components/Text2ImgSidebar.tsx:464-499](file://client/src/components/Text2ImgSidebar.tsx#L464-L499)
...
- **新增** 参考图像处理器：支持工作流 7 的参考图像上传、管理和ControlNet深度/姿态处理。
...
**更新** 新增 LoRA 处理器，支持多槽位 LoRA 模型的动态配置和智能连接。新增参考图像处理器，支持工作流 7 的参考图像管理与ControlNet深度/姿态处理。
...
participant RefHandler as "参考图像处理器"
...
Backend->>RefHandler : "处理参考图像如适用"
RefHandler-->>Backend : "返回带参考图像的模板"
...
### 快速出图（工作流 7）- 增强功能
- 模板来源：[ComfyUI_API/Pix2Real-二次元生成.json](file://ComfyUI_API/Pix2Real-二次元生成.json) 和 [ComfyUI_API/二次元生成 (PRO).json](file://ComfyUI_API/二次元生成 (PRO).json)
- 处理流程：支持两种模式：
  - 标准模式：纯文本到图像生成，使用二次元生成模板
  - PRO模式：支持参考图像，使用二次元生成 (PRO) 模板，支持ControlNet深度和姿态处理
- 参数配置：
  - 标准模式：输入提示词、可选 LoRA 配置、模型、尺寸、采样器等
  - PRO模式：输入提示词、参考图像、深度强度、姿态强度、可选 LoRA 配置等
- 输出效果：生成高质量的二次元风格图像，PRO模式下可保持参考图像的结构和姿态特征

**更新** 新增参考图像管理功能，支持ControlNet深度和姿态处理，增强图像生成的结构一致性。

**章节来源**
- [server/src/routes/workflow.ts:269-400](file://server/src/routes/workflow.ts#L269-L400)
- [ComfyUI_API/Pix2Real-二次元生成.json:1-145](file://ComfyUI_API/Pix2Real-二次元生成.json#L1-L145)
- [ComfyUI_API/二次元生成 (PRO).json:1-422](file://ComfyUI_API/二次元生成 (PRO).json#L1-L422)

### ZIT快出（工作流 9）- 增强功能
- 模板来源：[ComfyUI_API/Pix2Real-ZIT文生图NEW2.json](file://ComfyUI_API/Pix2Real-ZIT文生图NEW2.json)
- 处理流程：使用 UNet + LoRA + Shift 组合，支持 AuraFlow shift 功能与多槽位 LoRA 链接。
- 参数配置：输入提示词、UNet 模型、可选 LoRA 配置、Shift 开关与强度、模型、尺寸、采样器等。
- 输出效果：高质量的文本到图像生成，支持灵活的模型组合与风格控制。

**章节来源**
- [server/src/routes/workflow.ts:480-588](file://server/src/routes/workflow.ts#L480-L588)
- [ComfyUI_API/Pix2Real-ZIT文生图NEW2.json:1-226](file://ComfyUI_API/Pix2Real-ZIT文生图NEW2.json#L1-L226)
...
### 概述
系统现已全面集成多槽位 LoRA 支持，允许在单个工作流中同时使用最多五个 LoRA 模型。该功能通过动态节点重连逻辑、条件路由和智能链接功能实现，为用户提供更灵活的模型组合能力。

### 技术实现

#### 动态节点重连逻辑
系统为工作流 7、工作流 9 和二次元生成 (PRO) 实现了智能的 LoRA 节点重连机制：
...
- **二次元生成 (PRO)**：使用节点 #47 控制是否启用参考图像处理
...
- [ComfyUI_API/二次元生成 (PRO).json:327-421](file://ComfyUI_API/二次元生成 (PRO).json#L327-L421)

#### LoRA 链接功能
系统支持最多五个 LoRA 模型的链式连接：

1. **节点 ID 分配**：
   - 工作流 7：节点 50、51、52、53、54
   - 工作流 9：节点 36、50、51、52、53
   - 二次元生成 (PRO)：节点 70、71、72、73、74
...
#### 二次元生成 (PRO)（工作流 7 的 PRO 模式）
- **模板来源**：[ComfyUI_API/二次元生成 (PRO).json](file://ComfyUI_API/二次元生成 (PRO).json)
- **LoRA 节点**：70、71、72、73、74
- **功能特点**：
  - 支持最多 5 个 LoRA 模型
  - ControlNet深度和姿态处理
  - 参考图像管理
...
### 概述
系统新增了工作流 7 的参考图像管理功能，支持ControlNet深度和姿态处理，增强PRO模板模式的图像生成能力。用户可以通过拖拽或上传的方式添加参考图像，系统会自动检测图像尺寸并提供相应的比例控制。

### 技术实现

#### 参考图像上传与管理
系统为工作流 7 实现了完整的参考图像管理：

```mermaid
flowchart TD
Start["开始参考图像处理"] --> Upload["上传参考图像"]
Upload --> Validate["验证文件格式"]
Validate --> Save["保存到 zit_ref 目录"]
Save --> Detect["检测图像尺寸"]
Detect --> Return["返回文件信息"]
Return --> UseInWorkflow["在工作流中使用"]
UseInWorkflow --> ControlNet["ControlNet深度和姿态处理"]
ControlNet --> Generate["生成最终图像"]
Generate --> End["完成"]
```

**图表来源**
- [server/src/routes/workflow.ts:435-478](file://server/src/routes/workflow.ts#L435-L478)

#### ControlNet 深度和姿态处理
PRO模板支持以下ControlNet节点：

1. **深度处理（Depth）**：
   - 节点 48：ControlNetLoader（加载 illustriousXLDepth_v20.safetensors）
   - 节点 49：ControlNetApplyAdvanced（应用深度ControlNet）
   - 默认强度：0.3（可通过 depthStrength 参数调整）

2. **姿态处理（Pose）**：
   - 节点 58：ControlNetLoader（加载 illustriousXL_Openpose_v10.safetensors）
   - 节点 57：ControlNetApplyAdvanced（应用姿态ControlNet）
   - 默认强度：0.5（可通过 poseStrength 参数调整）

3. **姿态预处理**：
   - 节点 55：DWPreprocessor（姿态预处理器）
   - 支持手部、身体、面部检测
   - 使用 dw-ll_ucoco_384_bs5.torchscript.pt 模型

#### 参考图像处理流程
```mermaid
flowchart LR
ReferenceImage["参考图像"] --> LoadImage["加载图像节点"]
LoadImage --> Scale["缩放到总像素"]
Scale --> DepthCN["深度ControlNet处理"]
DepthCN --> PoseCN["姿态ControlNet处理"]
PoseCN --> Combine["组合处理结果"]
Combine --> Sampler["KSampler采样"]
Sampler --> Decode["VAE解码"]
Decode --> Output["输出图像"]
```

**图表来源**
- [ComfyUI_API/二次元生成 (PRO).json:181-281](file://ComfyUI_API/二次元生成 (PRO).json#L181-L281)

### API 接口

#### 参考图像管理接口
- **POST /api/workflow/7/ref-image**：上传参考图像
  - 请求：multipart/form-data，字段名为 image
  - 响应：{ filename, url, width, height }

- **GET /api/workflow/7/ref-image/:filename**：获取参考图像
  - 响应：图像文件（支持 PNG/JPG/WebP/GIF/BMP）

- **DELETE /api/workflow/7/ref-image/:filename**：删除参考图像
  - 响应：{ ok: true }

#### 参考图像参数
- **referenceImage**：参考图像文件名
- **depthStrength**：深度参考强度（0-1，默认0.3）
- **poseStrength**：姿态参考强度（0-1，默认0.5）
- **useOriginalRatio**：是否使用参考图像原始比例
- **refImageWidth/refImageHeight**：参考图像尺寸（可选）
...
**章节来源**
- [client/src/components/Text2ImgSidebar.tsx:464-499](file://client/src/components/Text2ImgSidebar.tsx#L464-L499)
- [client/src/components/Text2ImgSidebar.tsx:1334-1364](file://client/src/components/Text2ImgSidebar.tsx#L1334-L1364)

### 最佳实践

#### 参考图像选择
1. **图像质量**：选择清晰、主题明确的参考图像
2. **构图**：确保主体在图像中央，避免过度裁剪
3. **光线**：选择光线均匀、阴影适中的图像
4. **比例**：根据需要选择合适的图像比例

#### 强度参数调整
1. **深度强度**：0.3-0.5 是常用范围
2. **姿态强度**：0.4-0.7 是常用范围
3. **平衡调整**：深度和姿态强度需要协调使用

#### 错误处理
- 系统会自动验证参考图像文件是否存在
- 支持多种图像格式（PNG/JPG/WebP/GIF/BMP）
- 自动检测图像尺寸并提供比例控制
...
- **新增** LoRA 依赖：工作流 7、9 和二次元生成 (PRO) 依赖 LoRA 处理器进行动态配置和连接。
- **新增** 参考图像依赖：工作流 7 的 PRO 模式依赖参考图像处理器进行图像管理和ControlNet处理。
...
- **新增** 参考图像优化：使用 zit_ref 目录缓存参考图像，避免重复上传和处理。
...
- **新增** 参考图像相关问题：
  - 参考图像上传失败：检查 zit_ref 目录权限和磁盘空间
  - 参考图像不存在：确认文件名正确且未被删除
  - ControlNet处理失败：检查ControlNet模型文件是否正确安装
  - 强度参数无效：确认深度和姿态强度在 0-1 范围内
...
**更新** 多槽位 LoRA 支持的全面集成显著提升了系统的灵活性和功能性。通过动态节点重连逻辑、条件路由和智能链接功能，用户可以实现复杂的模型组合，满足各种创意需求。新增的参考图像管理功能进一步增强了工作流 7 的图像生成能力，支持ControlNet深度和姿态处理，使生成的图像更加符合参考图像的结构和姿态特征。
...
- **新增** 快速出图（7）与 ZIT快出（9）
  - 输入：文本提示词 + 可选 LoRA 配置
  - 可选：loras（数组，最多5个）、shiftEnabled（布尔）、shift（数值）
  - 示例：输入提示词"beautiful landscape"，启用两个 LoRA 模型，强度分别为0.7和0.5
- **新增** 快速出图（7）PRO模式
  - 输入：文本提示词 + 参考图像 + 可选 LoRA 配置
  - 可选：referenceImage（参考图像文件名）、depthStrength（深度强度0-1）、poseStrength（姿态强度0-1）、useOriginalRatio（使用原始比例）
  - 示例：输入提示词"beautiful girl"，选择参考图像，设置深度强度0.4，姿态强度0.6，使用原始比例
