...
**更新** 本系统现已升级至使用新的 Pix2Real-ZIT文生图NEW2.json 模板，改进了模型选择逻辑和提示词处理机制，支持更灵活的 UNet + LoRA + Shift 组合配置。
...
- 模板（ComfyUI_API）
  ...
  - **新增** Pix2Real-ZIT文生图NEW2.json 提供改进的 ZIT 文生图工作流，支持更灵活的模型组合
...
- 路由处理器
  ...
  - **更新** 路由层现已使用新的 Pix2Real-ZIT文生图NEW2.json 模板作为 ZIT 快出工作流的基础
- 服务层
...
**更新** 新的 ZIT 文生图工作流采用更灵活的模型组合架构，支持 UNet + LoRA + Shift 的动态切换，通过 ifElse 节点实现智能模型选择。
...
U->>FE : 设置参数并点击生成
FE->>BE : POST /api/workflow/9/execute
BE->>AD : 调用 buildPrompt(UNet/Lora/Shift 参数)
AD-->>BE : 返回已注入参数的 NEW2 模板 prompt
...
**图表来源**
- [server/src/routes/workflow.ts:235-309](file://server/src/routes/workflow.ts#L235-L309)
...
- 参数注入策略
  ...
  - **新增** ZIT 模板参数：UNet 模型、LoRA 模型、Shift 参数、模型选择开关
- 占位符与动态替换
  - 模板中不直接包含占位符语法；参数替换通过在运行时解析模板 JSON 并修改特定节点的 inputs 字段实现
  - 例如：将节点 "15" 的 image 字段设为上传后的文件名，将节点 "17" 的 prompt 字段设为基础提示词与用户提示词拼接结果
  - **更新** ZIT NEW2 模板支持 ifElse 节点进行智能模型选择，通过布尔值控制 LoRA 和 Shift 模式的切换
- 版本与兼容性
  - 代码通过适配器封装模板差异，新增工作流只需新增适配器，避免修改通用路由逻辑
  - 对于需要特殊连接的场景（如 LoRA/UNet/Shift 的链路切换），在路由层进行条件分支与连线重写
  - **新增** 从 Pix2Real-ZIT文生图NEW.json 迁移到 Pix2Real-ZIT文生图NEW2.json，提供更灵活的模型组合

```mermaid
flowchart TD
Start(["开始"]) --> Load["读取 NEW2 模板 JSON"]
Load --> InjectUNet["注入 UNet 模型到节点 25"]
InjectUNet --> InjectLora["注入 LoRA 模型到节点 36"]
InjectLora --> InjectShift["注入 Shift 参数到节点 45"]
InjectShift --> InjectImage["注入图像文件名到 LoadImage 节点"]
InjectImage --> InjectPrompt["注入提示词到节点 5"]
InjectPrompt --> InjectSampler["注入采样器/步数/CFG/种子等参数"]
InjectSampler --> ModelSwitch{"LoRA 开关状态"}
ModelSwitch --> |启用| EnableLora["ifElse 节点选择 LoRA 模型"]
ModelSwitch --> |禁用| DisableLora["绕过 LoRA，直接使用 UNet"]
EnableLora --> Done(["返回完整 prompt"])
DisableLora --> Done
```

**图表来源**
- [server/src/routes/workflow.ts:260-295](file://server/src/routes/workflow.ts#L260-L295)
- [ComfyUI_API/Pix2Real-ZIT文生图NEW2.json:22-288](file://ComfyUI_API/Pix2Real-ZIT文生图NEW2.json#L22-L288)
...
- 核心工作流模板
  ...
  - 通过适配器与通用路由实现统一入口
- 专业功能模板
  - 文本生图（7）、**更新** ZIT 快出（9）使用 NEW2 模板、提示词助手、提示词反推（Qwen/Florence/WD-14）、换脸、解除装备、自动识别等
  - 每个功能独立路由，参数注入更复杂，涉及多节点重连与输出路径定制
  - **新增** ZIT 快出工作流现在使用 NEW2 模板，支持更灵活的模型组合
- 自定义模板
...
- 模板加载
  ...
  - **更新** ZIT 快出工作流现在使用 Pix2Real-ZIT文生图NEW2.json 作为基础模板
- 缓存策略
...
- 性能优化
  ...
  - **新增** ZIT NEW2 模板的 ifElse 节点优化了模型选择性能，避免不必要的模型加载
...
- 调试与测试
  ...
  - **新增** 测试 ZIT 模板的模型切换功能，验证 LoRA 和 Shift 模式的正确切换
...
- [ComfyUI_API/Pix2Real-ZIT文生图NEW2.json:1-189](file://ComfyUI_API/Pix2Real-ZIT文生图NEW2.json#L1-L189)
...
- 内存与显存
  ...
  - **新增** ZIT NEW2 模板的 ifElse 节点减少了不必要的模型加载，提升了内存使用效率
- 并发与队列
...
- 常见问题
  ...
  - **新增** ZIT 模板模型切换异常：检查 ifElse 节点的布尔值设置和连接重写逻辑
- 定位方法
  ...
  - **新增** ZIT 模板调试：检查 UNet、LoRA、Shift 节点的连接状态，验证 ifElse 节点的模型选择逻辑
...
## 结论
本系统通过"模板 + 适配器 + 服务层 + 前端状态"的分层设计，实现了对多种工作流的统一管理与扩展。模板参数系统以节点 ID 为核心，通过运行时注入实现灵活替换；路由层与服务层提供了完善的执行、监控与资源管理能力。

**更新** 通过迁移到 Pix2Real-ZIT文生图NEW2.json 模板，系统现在支持更灵活的模型组合和智能切换机制，显著提升了 ZIT 文生图工作流的性能和用户体验。建议在生产环境中引入模板缓存、队列优先级策略与更细粒度的错误处理，以进一步提升稳定性与性能。
...
- 关键接口一览
  ...
  - **新增** POST /api/workflow/9/execute：使用 NEW2 模板的 ZIT 快出工作流
- 前端事件
