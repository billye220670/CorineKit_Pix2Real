...
**更新** 本版本新增了完整的批处理脚本自动化工具，提供一键启动、调试和停止开发环境的能力。新增的 `debug.bat` 提供交互式调试模式，`stop.bat` 提供完整的服务停止功能，进一步简化了开发工作流程。
...
- 统一开发脚本与批处理
  ...
  - stop.bat：扫描并终止占用 3000/5173/8188 端口的进程。
...
- 停止脚本（stop.bat）
  - 扫描并终止占用端口的进程，输出是否成功。
  - 分别检查服务器端口（3000）、客户端端口（5173）和 ComfyUI 端口（8188）。
  - 提供清晰的状态反馈，显示是否找到并终止了对应服务。
- 测试脚本（test.bat）
...
- 适配器模式
  - 每个 workflow 对应一个适配ator，加载 JSON 模板并按需打补丁（替换图像、提示词、种子等）。
- 与 ComfyUI 交互
...
- 端口占用
  ...
  - debug.bat 提供详细的端口检查功能，可单独运行以诊断端口问题。
- CORS 与代理
...
**更新** 通过 start.bat、debug.bat 和 stop.bat 的自动化脚本，开发者可以更便捷地管理开发环境，减少了手动操作的复杂性和出错概率。debug.bat 提供了交互式调试模式，stop.bat 提供了完整的服务停止功能，进一步完善了开发工具链。
...
- 调试技巧
  ...
  - stop.bat 可以同时停止所有相关服务，包括 ComfyUI，确保环境干净。
