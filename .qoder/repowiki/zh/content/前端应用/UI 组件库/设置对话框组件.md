...
## 更新摘要
**变更内容**
- 新增 Grok 反向提示词模型选项，扩展了可用的 AI 模型选择范围
- 更新了反向提示词模型配置，现在支持 Qwen3VL、Florence、WD-14 和 Grok 四种模型
- 完善了服务器端对 Grok 模型的支持，包括 API 调用和错误处理
...
**更新** 本版本新增了对 Grok 反向提示词模型的支持，扩展了用户可选择的 AI 模型范围，提供了更多样化的图像提示词反推能力。

## 项目结构
SettingsModal 组件位于客户端前端代码中，与全局状态管理、主题变量、会话管理等模块协同工作：
...
**章节来源**
- [SettingsModal.tsx:23-239](file://client/src/components/SettingsModal.tsx#L23-L239)
...
- 模型选择：反向提示词模型支持四种选择，包括新增的 Grok 模型。
...
participant Server as "workflow.ts"
User->>App : 点击设置图标
App->>Store : openSettings()
Store-->>App : settingsOpen = true
App->>SM : 渲染设置对话框
User->>SC : 选择 Grok 模型
SC->>Store : setReversePromptModel('Grok')
Store->>Store : localStorage.setItem(...)
Store-->>SM : 更新状态
User->>Server : 发送反向提示词请求
Server->>Server : 调用 Grok API
Server-->>User : 返回提示词结果
```
...
- 功能特性：
  ...
  - 设置项：工作流与会话两大类设置，分别使用 Segment 控件进行选择。
  - **更新** 反向提示词模型现在支持四种选择：Qwen3VL、Florence、WD-14 和新增的 Grok 模型。
...
## 结论
SettingsModal 通过清晰的状态管理、可复用的控件与良好的响应式设计，实现了简洁高效的设置界面。其基于 localStorage 的持久化策略与基于 IntersectionObserver 的滚动同步机制，确保了良好的用户体验。**更新** 新增的 Grok 反向提示词模型进一步增强了系统的 AI 能力，为用户提供了更多样化的图像分析选择。结合文档中的扩展指导，开发者可以轻松添加新的设置类别与项，实现更丰富的配置能力。
