...
## 更新摘要
**变更内容**
- 更新以集成新的阶段化进度跟踪：updateProgress 函数新增 stage、stepIndex、stepTotal 参数，支持更精细的进度显示
...
J["ProgressOverlay<br/>进度覆盖层"]
K["ImageCard<br/>图像卡片组件"]
end
subgraph "服务端"
L["sessionManager<br/>会话文件系统/状态JSON"]
M["agentService<br/>AI代理服务"]
N["agent路由<br/>AI代理API接口"]
end
A --> B
B --> D
D --> L
...
WS-->>Store : 收到进度/完成/错误消息(stage/stepIndex/stepTotal)
...
### 阶段化进度跟踪：updateProgress 函数增强
**更新** 阶段化进度跟踪功能已集成到 updateProgress 函数中，提供更精细的进度显示：

- 函数签名更新
  - updateProgress(promptId: string, percentage: number, stage?: string, stepIndex?: number, stepTotal?: number)
  - 新增三个可选参数：stage（当前阶段）、stepIndex（当前步骤索引）、stepTotal（总步骤数）

- 服务端阶段映射
  - 服务端根据节点 class_type 自动映射为中文阶段名称
  - 支持模型加载、采样、VAE解码等阶段的精确标识

- 客户端显示增强
  - ProgressOverlay 组件支持 stage、stepIndex、stepTotal 参数
  - 展示当前处理阶段和步骤进度（如"第3步/总12步"）
  - 提供更直观的进度可视化体验

- 状态更新机制
  - 收到进度消息时自动从 queued 推进为 processing
  - 支持跨标签页的状态同步和查找
  - 保持向后兼容性，未提供新参数时仍可正常工作
...
Server->>WS : 发送进度消息(stage/stepIndex/stepTotal)
WS->>Store : updateProgress(promptId, percentage, stage, stepIndex, stepTotal)
Store->>Store : 更新任务状态(progress/stage/stepIndex/stepTotal)
Store-->>UI : 通知状态变化
UI->>UI : 渲染阶段化进度显示
UI->>UI : 显示"第3步/总12步"等详细信息
```

**图表来源**
- [useWorkflowStore.ts:453-477](file://client/src/hooks/useWorkflowStore.ts#L453-L477)
- [useWebSocket.ts:40-42](file://client/src/hooks/useWebSocket.ts#L40-L42)
- [ProgressOverlay.tsx:12-125](file://client/src/components/ProgressOverlay.tsx#L12-L125)

**章节来源**
- [useWorkflowStore.ts:73](file://client/src/hooks/useWorkflowStore.ts#L73)
- [useWorkflowStore.ts:453-477](file://client/src/hooks/useWorkflowStore.ts#L453-L477)
- [useWebSocket.ts:40-42](file://client/src/hooks/useWebSocket.ts#L40-L42)
...
participant Store as "useWorkflowStore"
participant AgentStore as "useAgentStore"
WS-->>Hook : "connected/execution_start/progress/complete/error(stage/stepIndex/stepTotal)"
Hook->>Store : 调用相应方法(updateProgress含新参数)
...
WS-->>AgentStore : progress/complete/error(stage/stepIndex/stepTotal)
...
**图表来源**
- [useWorkflowStore.ts:96-722](file://client/src/hooks/useWorkflowStore.ts#L96-L722)
...
- **新增** 阶段化进度跟踪性能优化
  - updateProgress 函数支持可选参数，避免每次调用都必须传入新参数
  - ProgressOverlay 组件按需渲染阶段信息，减少不必要的DOM更新
  - 服务端阶段映射缓存，避免重复计算
...
- 任务状态不更新
  ...
  - **新增** 阶段化进度跟踪检查：确认 stage、stepIndex、stepTotal 参数是否正确传递
...
- **新增** removeImageByPromptId 不生效
  ...
  - 验证跨标签页的状态查找逻辑
- **新增** 阶段化进度显示异常
  - 检查服务端是否正确发送 stage、stepIndex、stepTotal 参数
  - 确认 ProgressOverlay 组件是否正确接收和渲染这些参数
  - 验证 updateProgress 函数的参数传递是否完整
- **新增** AI代理对话异常
...
- **新增** 阶段化进度字段：stage（当前阶段）、stepIndex（当前步骤）、stepTotal（总步骤数）
...
- **新增** 阶段化进度跟踪
  - 服务端自动发送 stage、stepIndex、stepTotal 参数
  - 客户端自动渲染详细的进度信息（如"第3步/总12步"）
  - ProgressOverlay 组件自动适配阶段化显示
- **新增** AI代理对话
...
- **新增** 阶段化进度跟踪中，确保服务端正确发送 stage、stepIndex、stepTotal 参数
- **新增** ProgressOverlay 组件按需渲染阶段信息，避免不必要的DOM更新
