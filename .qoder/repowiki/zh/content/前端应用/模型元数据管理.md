...
## 更新摘要
**变更内容**
- 新增超过100行的姿态类别条目，显著扩展了可用模型选项特别是姿态类别
...
**更新** 本次更新显著扩展了模型目录数据，新增了超过1725行的模型元数据，涵盖了角色、姿态、表情、风格、滑块和多视角等多个详细分类系统，包括来自不同游戏系列（如原神、碧蓝档案、鸣潮、探灵直播、魔女的复仇之夜等）的专业模型。特别地，姿态类别得到了大幅扩展，包含超过100个详细的姿势模型条目。
...
**章节来源**
- [metadata.json:1-1725](file://model_meta/metadata.json#L1-L1725)
...
**更新** 特别值得注意的是，系统现在包含了超过1725行的模型元数据，其中姿态类别得到了大幅扩展，涵盖了IL、PONY、光辉等多个系列的详细姿势模型，为用户提供了更加丰富和专业的模型选择体验。
