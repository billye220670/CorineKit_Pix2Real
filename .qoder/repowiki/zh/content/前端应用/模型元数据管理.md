...
## 更新摘要
**变更内容**
- 新增 LoRA 触发词管理系统，支持触发词的 CRUD 操作
- 新增模型分类管理功能，支持分类的创建、编辑和删除
- 在 ModelSelect 组件中添加触发词编辑和分类管理功能
- 在 Text2ImgSidebar 和 ZITSidebar 中集成触发词显示和复制功能
- 更新数据结构以支持触发词和分类字段
- 扩展了右键菜单功能，支持分类操作
...
**更新** 该系统现已扩展为支持 LoRA 触发词管理和模型分类管理，用户可以为 LoRA 模型设置和管理触发词，为模型设置分类标签，这些功能可以在生成过程中自动应用到提示词中，大大增强了模型管理的灵活性和智能化程度。
...
### 1. 数据存储层
- **metadata.json**: 存储所有模型的元数据信息，包括新的触发词和分类字段
...
### 3. 前端交互层
- **useModelMetadata Hook**: 提供模型元数据的状态管理和操作方法，包括触发词和分类管理
...
API->>Meta : 更新触发词信息
API-->>Client : 返回确认结果
Client->>API : POST /api/models/metadata/category
API->>Meta : 更新分类信息
API-->>Client : 返回确认结果
```

**图表来源**
- [modelMeta.ts:44-225](file://server/src/routes/modelMeta.ts#L44-L225)
- [useModelMetadata.ts:113-212](file://client/src/hooks/useModelMetadata.ts#L113-L212)
...
1. **元数据读取**: 从 JSON 文件中读取所有模型元数据，包括触发词和分类字段
2. **缩略图上传**: 支持多种图片格式的上传和管理
3. **昵称设置**: 允许用户为模型设置自定义显示名称
4. **触发词管理**: 新增触发词的设置和删除功能
5. **分类管理**: 新增分类的设置和删除功能
6. **文件清理**: 自动清理不再使用的缩略图文件

**章节来源**
- [modelMeta.ts:28-83](file://server/src/routes/modelMeta.ts#L28-L83)
- [modelMeta.ts:85-225](file://server/src/routes/modelMeta.ts#L85-L225)

### 前端 Hook 组件 (useModelMetadata.ts)

前端 Hook 组件提供了响应式的状态管理和异步操作，包括新增的触发词和分类管理功能：
...
**图表来源**
- [useModelMetadata.ts:8-214](file://client/src/hooks/useModelMetadata.ts#L8-L214)

#### 主要功能特性

1. **自动加载**: 组件挂载时自动从服务器加载元数据
2. **响应式更新**: 所有操作都会同步更新本地状态，包括触发词和分类
...
**图表来源**
- [ModelSelect.tsx:37-1005](file://client/src/components/ModelSelect.tsx#L37-L1005)
...
- [ModelSelect.tsx:800-1005](file://client/src/components/ModelSelect.tsx#L800-L1005)
...
- [useModelMetadata.ts:113-153](file://client/src/hooks/useModelMetadata.ts#L113-L153)
...
- [useModelMetadata.ts:113-128](file://client/src/hooks/useModelMetadata.ts#L113-L128)
...
**图表来源**
- [Text2ImgSidebar.tsx:329-369](file://client/src/components/Text2ImgSidebar.tsx#L329-L369)
...
- [useModelMetadata.ts:156-196](file://client/src/hooks/useModelMetadata.ts#L156-L196)
...
**图表来源**
- [ModelSelect.tsx:8-70](file://client/src/components/ModelSelect.tsx#L8-L70)
...
**更新** 本次更新显著增强了系统的功能，特别是新增的 LoRA 触发词管理系统和模型分类管理系统，为用户提供了更强大的模型管理能力。触发词功能的加入使得用户可以为 LoRA 模型设置特定的触发词，这些触发词可以在生成过程中自动应用，大大提高了生成质量和效率。分类管理功能的加入使得用户可以按照不同的维度对模型进行组织和筛选，提升了用户体验和工作效率。

### 主要优势

1. **易用性**: 直观的 UI 设计和丰富的交互功能，包括触发词编辑和分类管理
2. **可靠性**: 完善的错误处理和数据验证机制
3. **扩展性**: 模块化的架构设计便于功能扩展
4. **性能**: 优化的文件存储和网络传输策略
5. **专业性**: 专业的 LoRA 触发词管理和分类管理功能
6. **智能化**: 触发词和分类功能的智能集成，提升生成质量
