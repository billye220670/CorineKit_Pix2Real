...
## 更新摘要
**变更内容**
- 新增阶段化进度跟踪系统，提供更详细的进度信息
- 实现加权进度计算，基于节点权重的全局进度追踪
- 增强进度状态管理，支持阶段映射和详细进度指标
- 更新工作流状态以支持新的进度数据结构
...
**更新** 新增阶段化进度跟踪系统，提供基于节点权重的精确进度计算和阶段映射功能。
...
- **进度跟踪系统（Progress Tracking）** **新增**
  - 基于节点权重的全局进度计算，提供精确的进度追踪。
  - 阶段映射系统，将节点类型转换为中文阶段名称。
  - 详细进度信息，包括当前阶段、步骤索引和总步骤数。
...
**更新** 新增的进度跟踪系统通过 WebSocket 服务器实时计算和推送阶段化进度信息，提供更精确的工作流执行状态。
...
- 关键流程
  ...
  - **更新** 通过 WebSocket 接收阶段化进度信息，更新任务状态和进度显示。
- 性能与可靠性
...
### 进度跟踪系统（Progress Tracking）**新增**
- 设计要点
  - **PromptProgressState 接口**：定义阶段化进度状态，包括总节点数、总权重、已完成权重、当前节点权重、当前值、当前最大值等。
  - **加权进度计算**：基于节点权重的全局进度计算公式：`(已完成权重 + 当前节点权重 × 当前节点内部进度) / 总权重`。
  - **阶段映射**：将 ComfyUI 节点类型映射为中文阶段名称，如"加载主模型"、"采样中"、"VAE 解码"等。
  - **详细进度信息**：支持当前阶段名称、步骤索引（1-based）和总步骤数的完整进度追踪。
- 核心功能
  - 节点切换处理：当节点切换时，将上一节点的完整权重计入已完成权重。
  - 缓存命中处理：缓存命中的节点直接跳过，权重计入已完成权重。
  - 实时进度推送：通过 WebSocket 向客户端推送精确的进度信息。
- 数据结构
  - `PromptProgressState`：包含进度计算所需的所有状态信息。
...
SetCurrent --> EmitProgress["计算并推送进度"]
NodeChange --> |否| CheckProgress["检查进度更新"]
CheckProgress --> EmitProgress
CacheHit["缓存命中"] --> AddWeight["将缓存节点权重计入已完成权重"]
AddWeight --> UpdateStep
EmitProgress --> Wait["等待下一步事件"]
```
...
- **进度显示异常** **新增**
  ...
  - 验证阶段映射配置，确保节点类型能够正确映射到中文名称。
  - 确认权重计算逻辑，检查节点权重设置是否合理。
  - 检查进度状态清理机制，避免内存泄漏。
...
**更新** 新增的阶段化进度跟踪系统显著提升了用户体验，通过基于节点权重的精确进度计算和阶段映射，为用户提供了更直观、更准确的工作流执行状态反馈。该系统与现有的状态管理架构无缝集成，保持了系统的整体性和一致性。
...
- Hook 使用模式
  ...
  - **进度状态使用**：const { progress, stage, stepIndex, stepTotal } = useWorkflowStore(s => s.tabData[tab].tasks[imageId])
- 最佳实践
  ...
  - **进度跟踪最佳实践**：合理设置节点权重，确保进度计算的准确性；及时清理进度状态，避免内存泄漏。