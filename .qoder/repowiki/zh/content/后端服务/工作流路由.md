...
### 组件二：批量处理接口 POST /api/workflow/:id/batch
- 设计思路
...
### 组件三：任务状态查询接口 GET /api/workflow/:id/status（设计建议）
- 现状
...
### 组件五：与 ComfyUI 的集成与 WebSocket 状态监听
- 上传与入队
...
## 结论
工作流路由模块通过清晰的分层设计与适配器模式，实现了对多种工作流的统一接入与扩展。当前已具备完善的单图与批量执行能力，并通过服务层与 WebSocket 提供了良好的状态反馈基础。后续可在“任务状态查询接口”“批量并发控制”“历史记录持久化”等方面进一步完善，以满足更高吞吐与更丰富的用户体验需求。
