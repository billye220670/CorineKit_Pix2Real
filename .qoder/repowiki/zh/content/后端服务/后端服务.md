...
- **新增**：智能AI代理服务，集成LLM驱动的图像生成、配置管理和智能问答功能
- **新增**：自动 ComfyUI 服务管理与启动，增强服务器启动过程的可靠性
...
- routes：REST 路由，包含工作流执行、输出文件列表与下载、会话读写、**新增** AI代理服务
- services：业务服务，封装 ComfyUI 的 HTTP/WebSocket 客户端与会话管理，**新增** LLM服务、AI代理服务、意图解析服务、用户画像服务，**新增** ComfyUI Launcher 服务
...
S3["服务: comfyuiLauncher.ts"]
S4["服务: llmService.ts"]
S5["服务: agentService.ts"]
S6["服务: intentParser.ts"]
S7["服务: profileService.ts"]
...
**图表来源**
- [server/src/index.ts:1-252](file://server/src/index.ts#L1-L252)
...
- [server/src/routes/agent.ts:1-1605](file://server/src/routes/agent.ts#L1-L1605)
...
- [server/src/services/llmService.ts:1-877](file://server/src/services/llmService.ts#L1-L877)
...
- **新增** AI代理服务：集成LLM驱动的智能图像生成、配置管理和智能问答功能，支持多模式交互（智能体、配置助理、智能问答）。
- **新增** LLM服务：提供统一的LLM调用接口，支持Function Calling工具调用，集成智能LoRA推荐和冲突检测功能。
- **新增** 意图解析服务：将LLM的Function Calling结果解析为具体的工作流参数，支持LoRA匹配、模型推荐和参数优化。
- **新增** 用户画像服务：基于历史生成记录构建用户偏好画像，为AI代理提供个性化建议。
- **新增** ComfyUI Launcher 服务：自动检测、启动和管理 ComfyUI 服务，确保服务器启动时 ComfyUI 已就绪。
...
- [server/src/services/comfyuiLauncher.ts:13-131](file://server/src/services/comfyuiLauncher.ts#L13-L131)
- [server/src/services/llmService.ts:1-877](file://server/src/services/llmService.ts#L1-L877)
- [server/src/services/agentService.ts:1-118](file://server/src/services/agentService.ts#L1-118)
- [server/src/services/intentParser.ts:1-641](file://server/src/services/intentParser.ts#L1-L641)
- [server/src/services/profileService.ts:1-238](file://server/src/services/profileService.ts#L1-L238)
...
- **新增**：调用 ensureComfyUI() 自动检测并启动 ComfyUI 服务
...
participant Launcher as "ComfyUI 启动器"
participant WS as "WebSocket 服务器"
participant CWS as "ComfyUI WebSocket"
participant HTTP as "ComfyUI HTTP"
Browser->>Server : "GET /api/workflow/ : id/execute"
Server->>Launcher : "ensureComfyUI()"
Launcher->>HTTP : "检测服务状态"
alt 服务未运行
Launcher->>Launcher : "launchComfyUI()"
Launcher->>HTTP : "轮询等待就绪"
end
...
**图表来源**
- [server/src/index.ts:234-252](file://server/src/index.ts#L234-L252)
...
### AI代理服务与LLM集成（新增）
**更新** 新增强大的AI代理服务，集成智能图像生成、配置管理和智能问答功能

AI代理服务提供以下核心功能：
- **多模式交互**：支持智能体模式（图像生成）、配置助理模式（参数调整）、智能问答模式（技术咨询）
- **LLM驱动**：集成Grok API，支持Function Calling工具调用，实现智能决策和参数优化
- **智能LoRA推荐**：基于用户画像和提示词，自动推荐最合适的LoRA模型和权重
- **冲突检测**：在配置助理模式下检测LoRA触发词冲突，提供多种解决方案
- **用户画像**：基于历史生成记录构建用户偏好画像，提供个性化建议
- **意图解析**：将LLM的Function Calling结果解析为具体的工作流参数

```mermaid
flowchart TD
Start(["用户请求"]) --> Mode{"选择模式"}
Mode --> |智能体| Agent["AI智能体模式"]
Mode --> |配置助理| Config["配置助理模式"]
Mode --> |智能问答| QA["智能问答模式"]
Agent --> Profile["构建用户画像"]
Profile --> LLM1["LLM Function Calling"]
LLM1 --> Parse["意图解析"]
Parse --> Workflow["工作流执行"]
Config --> Protect["触发词保护"]
Protect --> Conflict{"冲突检测"}
Conflict --> |无冲突| Apply["应用配置变更"]
Conflict --> |有冲突| Report["报告冲突方案"]
QA --> Direct["直接问答"]
```

**图表来源**
- [server/src/routes/agent.ts:755-1175](file://server/src/routes/agent.ts#L755-L1175)
- [server/src/services/llmService.ts:118-172](file://server/src/services/llmService.ts#L118-L172)
- [server/src/services/intentParser.ts:487-641](file://server/src/services/intentParser.ts#L487-L641)
- [server/src/services/profileService.ts:77-238](file://server/src/services/profileService.ts#L77-L238)
...
### ComfyUI Launcher 服务（新增）
**更新** 新增自动 ComfyUI 服务管理功能，显著提升服务器启动可靠性

ComfyUI Launcher 服务提供以下核心功能：
- **服务检测**：通过 HTTP 请求 /system_stats 接口检测 ComfyUI 是否正在运行
- **自动启动**：当检测到服务未运行时，自动启动 ComfyUI 进程
- **进程管理**：使用 child_process.spawn 启动 ComfyUI，支持环境变量覆盖安装路径
- **健康检查**：轮询等待服务就绪，设置最大等待时间和超时处理
- **状态查询**：提供 /api/comfyui/status 接口供前端查询服务状态

```mermaid
flowchart TD
Start(["服务器启动"]) --> Ensure["ensureComfyUI()"]
Ensure --> Detect{"isComfyUIRunning()"}
Detect --> |true| Success["✅ 服务已在运行"]
Detect --> |false| Launch["launchComfyUI()"]
Launch --> Wait["等待服务就绪 (轮询)"]
Wait --> Ready{"服务就绪?"}
Ready --> |true| Success
Ready --> |false| Timeout["抛出启动超时错误"]
Success --> End(["继续启动 Express 服务器"])
Timeout --> End
```
...
**章节来源**
- [server/src/services/comfyuiLauncher.ts:13-131](file://server/src/services/comfyuiLauncher.ts#L13-L131)
- [server/src/index.ts:64-71](file://server/src/index.ts#L64-L71)
- [server/src/index.ts:235-242](file://server/src/index.ts#L235-L242)
...
- 路由分组：
  ...
  - **新增** /api/agent：AI代理服务，包括智能体、配置助理、智能问答、生成历史、收藏管理等
  - **新增** /api/comfyui：ComfyUI 服务状态查询
...
- [server/src/routes/session.ts:18-95](file://server/src/routes/session.ts#L18-L95)
- [server/src/routes/agent.ts:611-1175](file://server/src/routes/agent.ts#L611-L1175)

### 错误处理机制
- 统一错误响应：捕获异常后返回 500 或 400，并包含错误消息
- ComfyUI 可用性降级：系统统计与队列查询失败时返回 502/504
- **新增** ComfyUI 启动错误：自动启动失败时记录错误日志，但不影响服务器继续运行
- **新增** LLM调用错误：智能代理服务中的LLM调用失败时提供友好的错误提示
- **新增** 配置助理冲突处理：LoRA冲突检测失败时提供兜底方案
...
- [server/src/index.ts:237-242](file://server/src/index.ts#L237-L242)
- [server/src/routes/agent.ts:1584-1599](file://server/src/routes/agent.ts#L1584-L1599)
...
- 模块耦合：
  ...
  - **新增**：agent路由依赖llmService、agentService、intentParser、profileService
  - **新增**：index.ts 依赖 comfyuiLauncher 服务
  - 服务之间低耦合，通过类型接口通信
...
Express --> RouterA["agent 路由"]
Express --> Launcher["comfyuiLauncher 服务"]
RouterW --> Adapter["适配器集合"]
RouterW --> ServiceC["comfyui 服务"]
RouterO --> ServiceS["sessionManager 服务"]
RouterS --> ServiceS
RouterA --> ServiceLLM["llmService 服务"]
RouterA --> ServiceAgent["agentService 服务"]
RouterA --> ServiceIntent["intentParser 服务"]
RouterA --> ServiceProfile["profileService 服务"]
ServiceC --> ComfyUI["ComfyUI HTTP/WS"]
Launcher --> ComfyUI
```

**图表来源**
- [server/src/index.ts:8-13](file://server/src/index.ts#L8-L13)
...
- **新增** LLM缓存：模型元数据缓存1分钟，减少文件读取开销
- **新增** 智能LoRA推荐：通过LLM自动匹配LoRA，减少人工配置时间
- **新增** 冲突检测：在配置助理模式下自动检测LoRA冲突，避免无效配置
- **新增** 用户画像：基于历史数据提供个性化建议，提升用户体验
- **新增** ComfyUI 启动优化：自动启动过程包含超时控制和错误处理，避免阻塞主服务器启动

## 故障排查指南
- ComfyUI 不可用
  - 现象：系统统计/队列/入队返回 502/504
  - 排查：确认 ComfyUI 在 http://127.0.0.1:8188 运行
  - **新增**：检查 /api/comfyui/status 状态，确认自动启动是否成功
- **新增** ComfyUI 启动失败
  - 现象：服务器启动时显示自动启动失败警告
  - 排查：检查 COMFYUI_PATH 环境变量，确认 Python 可执行文件路径正确
  - 排查：查看启动日志中的错误信息，确认 ComfyUI 主程序存在
- **新增** AI代理服务异常
  - 现象：智能体、配置助理或智能问答功能失效
  - 排查：检查LLM API密钥和Grok服务可用性
  - 排查：确认模型元数据文件存在且格式正确
  - 排查：检查用户画像数据读取权限
- **新增** LoRA冲突检测失败
  - 现象：配置助理模式下冲突检测异常
  - 排查：检查LoRA元数据中的触发词字段格式
  - 排查：确认用户画像中的LoRA使用记录完整
- WebSocket 连接失败
...
- [server/src/services/comfyuiLauncher.ts:117-129](file://server/src/services/comfyuiLauncher.ts#L117-L129)
- [server/src/routes/agent.ts:1584-1599](file://server/src/routes/agent.ts#L1584-L1599)

## 结论
该后端服务以适配器模式组织工作流，结合 Express 的路由与中间件、ComfyUI 的 HTTP/WebSocket 能力以及本地会话持久化，形成一套完整的本地图像/视频批处理与实时反馈系统。**新增的智能AI代理服务集成了LLM驱动的图像生成、配置管理和智能问答功能，通过多模式交互和智能决策大大提升了用户体验**。**新增的 ComfyUI Launcher 服务显著提升了服务器启动的可靠性，通过自动检测、启动和健康检查机制，确保 ComfyUI 服务始终处于可用状态**。其模块化设计便于扩展更多工作流与功能，同时通过 WebSocket 事件桥接与输出下载优化提升了用户体验。
...
### AI代理服务路由（新增）
- 获取暖场建议
  - GET /api/agent/suggestions?mode=agent|config_assistant|smart_qa
  - 返回：{ suggestions: string[] }
- 记录生成日志
  - POST /api/agent/log-generation
  - JSON：{ sessionId, id, workflowId, workflowName, config, result, metadata }
  - 返回：{ ok }
- 获取生成历史
  - GET /api/agent/generation-history?sessionId=string
  - 返回：GenerationRecord[]
- 收藏/取消收藏
  - POST /api/agent/favorite
  - JSON：{ sessionId, imageId, tabId, isFavorited }
  - 返回：{ ok }
- 获取收藏列表
  - GET /api/agent/favorites?sessionId=string
  - 返回：Record<string, { tabId: number; favoritedAt: number }>
- 获取用户偏好画像
  - GET /api/agent/user-profile
  - 返回：UserPreferenceProfile
- AI对话 + 意图解析
  - POST /api/agent/chat
  - JSON：{ sessionId, message, messages, images, hasImage, mode, currentConfig, allowLoraModification }
  - 返回：多模式响应（text_response、tool_call、config_change、lora_conflict）
- 执行AI代理意图
  - POST /api/agent/execute
  - JSON：{ intent, clientId, sessionId }
  - 返回：工作流执行结果
...
### LLM服务接口（新增）
- LLM调用
  - POST /api/agent/chat（通过LLM服务）
  - 支持Function Calling工具调用
  - 返回：LLM响应或工具调用结果
- 智能LoRA推荐
  - POST /api/agent/chat?mode=config_assistant
  - 自动匹配LoRA并生成触发词
  - 返回：config_change响应
- 冲突检测
  - POST /api/agent/chat?mode=config_assistant
...
- [server/src/services/llmService.ts:118-172](file://server/src/services/llmService.ts#L118-L172)
- [server/src/services/llmService.ts:174-187](file://server/src/services/llmService.ts#L174-L187)
- [server/src/services/llmService.ts:191-301](file://server/src/services/llmService.ts#L191-L301)

### 意图解析接口（新增）
- LoRA匹配
  - POST /api/agent/chat（通过intentParser）
  - 基于提示词和用户画像匹配LoRA
  - 返回：ParsedIntent
- 模型推荐
  - POST /api/agent/chat（通过intentParser）
...
**章节来源**
- [server/src/services/intentParser.ts:38-89](file://server/src/services/intentParser.ts#L38-L89)
...
**章节来源**
- [server/src/services/profileService.ts:77-238](file://server/src/services/profileService.ts#L77-L238)
- [server/src/routes/agent.ts:345-466](file://server/src/routes/agent.ts#L345-L466)