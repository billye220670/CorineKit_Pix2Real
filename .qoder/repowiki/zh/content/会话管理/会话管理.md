# 会话管理

<cite>
**本文引用的文件**
- [server/src/services/sessionManager.ts](file://server/src/services/sessionManager.ts)
- [server/src/routes/session.ts](file://server/src/routes/session.ts)
- [client/src/services/sessionService.ts](file://client/src/services/sessionService.ts)
- [client/src/hooks/useSession.ts](file://client/src/hooks/useSession.ts)
- [client/src/hooks/useWorkflowStore.ts](file://client/src/hooks/useWorkflowStore.ts)
- [client/src/hooks/useMaskStore.ts](file://client/src/hooks/useMaskStore.ts)
- [client/src/components/SessionBar.tsx](file://client/src/components/SessionBar.tsx)
- [client/src/components/WelcomePage.tsx](file://client/src/components/WelcomePage.tsx)
- [server/src/index.ts](file://server/src/index.ts)
- [client/src/types/index.ts](file://client/src/types/index.ts)
- [server/src/types/index.ts](file://server/src/types/index.ts)
- [TODO-session-persistence.md](file://TODO-session-persistence.md)
- [sessions/187f40a5-66a0-4b2f-95fb-d6c9b6baeba6/session.json](file://sessions/187f40a5-66a0-4b2f-95fb-d6c9b6baeba6/session.json)
- [README.md](file://README.md)
</cite>

## 更新摘要
**变更内容**
- 新增手动封面图片功能，包括新的 /api/session/:sessionId/cover 端点
- 会话封面管理服务，支持封面图片复制和自动封面再生保护
- 会话状态新增 manualCover 和 coverExt 字段，用于跟踪手动封面状态
- 欢迎页面增强，支持手动封面显示和自动封面再生保护机制
- 新增 setSessionCover API，提供完整的封面管理能力

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性阐述 CorineKit Pix2Real 的会话管理系统，覆盖会话持久化机制（数据结构、文件存储、状态恢复）、会话 API 设计与实现（创建、序列化、清理）、per-tab 图像隔离的设计与实现（状态隔离、数据同步、资源管理），以及新增的手动封面图片功能。系统提供完整的会话管理能力，包括自动封面生成、手动封面设置、封面图片复制和自动封面再生保护机制。

**更新** 本次更新新增了手动封面图片功能，用户可以通过选择任意会话中的图片作为会话封面，系统会自动复制图片到会话根目录并标记为手动封面，防止后续自动封面再生覆盖。

## 项目结构
会话管理涉及前后端协作：前端负责状态序列化、增量上传与自动保存；后端负责目录与文件持久化、会话状态 JSON 管理、静态文件服务与清理策略。新增的手动封面功能通过专门的 API 端点实现，支持从会话中的输入图或输出图复制图片作为封面。

```mermaid
graph TB
subgraph "客户端"
UI["界面组件<br/>SessionBar.tsx, WelcomePage.tsx"]
Hook["会话钩子<br/>useSession.ts"]
WF["工作流状态<br/>useWorkflowStore.ts"]
MSK["遮罩状态<br/>useMaskStore.ts"]
SVC["会话服务封装<br/>sessionService.ts"]
WEL["欢迎页面<br/>WelcomePage.tsx"]
COVER["封面管理<br/>setSessionCover()"]
END
subgraph "服务端"
ROUTER["会话路由<br/>routes/session.ts"]
SMGR["会话管理器<br/>services/sessionManager.ts"]
SAVECOVER["封面保存<br/>saveCover()"]
IDX["入口与静态服务<br/>index.ts"]
END
UI --> Hook
Hook --> SVC
SVC --> ROUTER
ROUTER --> SMGR
ROUTER --> SAVECOVER
SMGR --> SAVECOVER
IDX --> ROUTER
Hook --> WF
Hook --> MSK
WEL --> SVC
WEL --> SMGR
COVER --> SVC
COVER --> ROUTER
```

**图表来源**
- [client/src/components/SessionBar.tsx:1-381](file://client/src/components/SessionBar.tsx#L1-L381)
- [client/src/components/WelcomePage.tsx:1-510](file://client/src/components/WelcomePage.tsx#L1-L510)
- [client/src/hooks/useSession.ts:1-422](file://client/src/hooks/useSession.ts#L1-L422)
- [client/src/services/sessionService.ts:1-157](file://client/src/services/sessionService.ts#L1-L157)
- [server/src/routes/session.ts:1-114](file://server/src/routes/session.ts#L1-L114)
- [server/src/services/sessionManager.ts:1-213](file://server/src/services/sessionManager.ts#L1-L213)
- [server/src/index.ts:1-228](file://server/src/index.ts#L1-L228)

**章节来源**
- [README.md: 项目结构与特性:41-62](file://README.md#L41-L62)
- [TODO-session-persistence.md: 会话持久化目标与策略:1-120](file://TODO-session-persistence.md#L1-L120)

## 核心组件
- 会话管理器（后端）
  - 目录与文件操作：确保会话目录、输入图、遮罩、输出文件保存与安全命名
  - 会话状态：序列化/反序列化 session.json，含创建/更新时间、活动标签页、各标签页数据
  - **新增** 手动封面管理：saveCover 函数实现封面图片复制和状态标记
  - 清理策略：列出会话、删除会话、裁剪旧会话
- 会话路由（后端）
  - 提供上传输入图、上传遮罩、保存/加载/删除会话状态与会话列表
  - **新增** 手动封面端点：POST /api/session/:sessionId/cover
- 会话服务封装（前端）
  - 类型化 API 封装：上传图片/遮罩、保存/加载/删除会话、列出会话
  - **新增** 手动封面 API：setSessionCover 函数
- 会话钩子（前端）
  - 自动保存：基于 store 订阅的防抖保存、页面卸载前 flush
  - 状态恢复：从服务器拉取 session.json，重建 ImageItem 与遮罩，恢复 store 与遮罩状态
  - per-tab 隔离：按 tab-0..9 维度组织输入图与遮罩
- 工作流与遮罩状态（前端）
  - 工作流状态：包含 images、prompts、tasks、selectedOutputIndex、backPoseToggles、配置等
  - 遮罩状态：以键值对存储遮罩像素数据，支持批量恢复
- 欢迎页面（前端）
  - **新增** 手动封面支持：当 meta.manualCover 为 true 时直接使用手动封面
  - **新增** 自动封面再生保护：手动封面设置后不会被后续自动封面覆盖
  - **新增** 增强的会话卡片：显示会话名称、更新时间、预览图像、图像计数和封面状态
- 入口与静态服务（后端）
  - 确保 sessions 目录存在，静态服务映射 /api/session-files → sessions 目录

**章节来源**
- [server/src/services/sessionManager.ts: 会话管理器接口与实现:1-213](file://server/src/services/sessionManager.ts#L1-L213)
- [server/src/routes/session.ts: 会话路由定义:1-114](file://server/src/routes/session.ts#L1-L114)
- [client/src/services/sessionService.ts: 前端会话 API 封装:1-157](file://client/src/services/sessionService.ts#L1-L157)
- [client/src/hooks/useSession.ts: 会话钩子与自动保存/恢复:1-422](file://client/src/hooks/useSession.ts#L1-L422)
- [client/src/hooks/useWorkflowStore.ts: 工作流状态与恢复:1-645](file://client/src/hooks/useWorkflowStore.ts#L1-L645)
- [client/src/hooks/useMaskStore.ts: 遮罩状态:1-51](file://client/src/hooks/useMaskStore.ts#L1-L51)
- [client/src/components/WelcomePage.tsx: 欢迎页面与图像计数:1-510](file://client/src/components/WelcomePage.tsx#L1-L510)
- [server/src/index.ts: 静态服务与会话目录:37-61](file://server/src/index.ts#L37-L61)

## 架构总览
会话管理采用"事件驱动静默自动保存"策略：导入图片立即复制到会话目录；任务完成更新 session.json；遮罩绘制结束保存 PNG；提示词变更 500ms 防抖更新；页面关闭前 flush。新增的手动封面功能通过专门的 API 端点实现，支持从会话中的任意图片设置为封面。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "界面(SessionBar, WelcomePage)"
participant Hook as "useSession"
participant Svc as "sessionService"
participant R as "session 路由"
participant M as "sessionManager"
participant SC as "saveCover"
participant FS as "文件系统(sessions/)"
participant WF as "工作流状态"
participant MSK as "遮罩状态"
U->>UI : "新建/切换/删除会话"
UI->>Hook : "触发新建/切换/删除"
Hook->>Svc : "调用 API"
Svc->>R : "POST/PUT/GET/DELETE /api/session/*"
R->>M : "执行保存/加载/删除"
M->>FS : "写入/读取 session.json 与文件"
Hook->>WF : "恢复/更新工作流状态"
Hook->>MSK : "恢复/更新遮罩状态"
Note over Hook,FS : "自动保存：订阅 store 变更，防抖写入 session.json"
Note over UI,FS : "欢迎页面：统计图像数量，增强会话卡片显示"
Note over U,SC : "手动封面：POST /api/session/ : sessionId/cover"
SC->>FS : "复制图片到 cover{ext}"
SC->>M : "更新 session.json 标记 manualCover=true"
```

**图表来源**
- [client/src/components/SessionBar.tsx:66-71](file://client/src/components/SessionBar.tsx#L66-L71)
- [client/src/components/WelcomePage.tsx:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
- [client/src/hooks/useSession.ts:164-181](file://client/src/hooks/useSession.ts#L164-L181)
- [client/src/services/sessionService.ts:103-121](file://client/src/services/sessionService.ts#L103-L121)
- [server/src/routes/session.ts:18-111](file://server/src/routes/session.ts#L18-L111)
- [server/src/services/sessionManager.ts:91-120](file://server/src/services/sessionManager.ts#L91-L120)
- [server/src/services/sessionManager.ts:165-204](file://server/src/services/sessionManager.ts#L165-L204)

## 详细组件分析

### 会话管理器（后端）
- 数据结构
  - SessionState：sessionId、createdAt、updatedAt、activeTab、tabData、**新增** manualCover、**新增** coverExt
  - SerializedTabData：images、prompts、tasks、selectedOutputIndex、backPoseToggles、配置字段
  - SerializedImage、SerializedTask：用于序列化存储的数据载体
- 文件存储策略
  - 目录：sessions/<sessionId>/tab-<n>/input、masks、output
  - 输入图：按扩展名保存，返回持久化 URL
  - 遮罩：maskKey 中冒号替换为下划线，避免 Windows 非法字符
  - 输出文件：下载至 session output 目录，返回 URL
  - **新增** 手动封面：复制图片到 sessions/<sessionId>/cover{ext}
- 状态恢复算法
  - 读取 session.json，逐标签页重建 images（fetch → File → Blob URL），恢复 tasks、selectedOutputIndex、backPoseToggles、配置
  - 遮罩探测：按 maskKey 与安全名尝试 HEAD 请求，存在则下载还原为 RGBA 像素
  - **新增** 封面恢复：如果 manualCover 为 true，直接使用 cover{ext} 作为封面
- 清理维护
  - 列出会话：按 updatedAt 倒序，**新增** 支持手动封面标记
  - 删除会话：递归删除目录
  - 裁剪旧会话：仅保留最近 N 个

```mermaid
flowchart TD
Start(["开始"]) --> Read["读取 session.json"]
Read --> CheckCover{"manualCover?"}
CheckCover --> |是| UseCover["使用手动封面 cover{ext}"]
CheckCover --> |否| ForTabs{"遍历标签页"}
ForTabs --> |存在| BuildImages["重建 ImageItem<br/>fetchAsFile()"]
BuildImages --> ProbeMasks["探测 masks<br/>HEAD 请求"]
ProbeMasks --> RestoreTasks["恢复 tasks/索引/配置"]
RestoreTasks --> ForTabs
ForTabs --> |不存在| CountImages["统计图像数量<br/>输入图像 + 输出图像"]
CountImages --> Done(["完成"])
UseCover --> Done
```

**图表来源**
- [server/src/services/sessionManager.ts:112-120](file://server/src/services/sessionManager.ts#L112-L120)
- [client/src/hooks/useSession.ts:316-366](file://client/src/hooks/useSession.ts#L316-L366)
- [server/src/services/sessionManager.ts:165-204](file://server/src/services/sessionManager.ts#L165-L204)

**章节来源**
- [server/src/services/sessionManager.ts: 数据结构与 I/O:61-120](file://server/src/services/sessionManager.ts#L61-L120)
- [server/src/services/sessionManager.ts: 目录与文件保存:10-57](file://server/src/services/sessionManager.ts#L10-L57)
- [server/src/services/sessionManager.ts: 列表/删除/裁剪:124-164](file://server/src/services/sessionManager.ts#L124-L164)
- [client/src/hooks/useSession.ts: 恢复流程:316-366](file://client/src/hooks/useSession.ts#L316-L366)

### 会话 API（前后端）
- 客户端 API
  - 上传输入图：multipart/form-data，返回持久化 URL
  - 上传遮罩：PNG，保存为 masks/<safeName>.png
  - 保存/加载/删除会话：PUT/GET/DELETE /api/session/:sessionId/state 与 /api/session/:sessionId
  - 列出最近会话：GET /api/session
  - **新增** 设置会话封面：POST /api/session/:sessionId/cover
  - **新增** 手动封面 API：setSessionCover 函数
- 服务端路由
  - POST /api/session/:sessionId/images：校验参数，保存输入图
  - POST /api/session/:sessionId/masks：保存遮罩
  - PUT/POST /api/session/:sessionId/state：保存 session.json（支持 sendBeacon）
  - GET /api/session/:sessionId、GET /api/session、DELETE /api/session/:sessionId
  - **新增** POST /api/session/:sessionId/cover：设置会话封面

```mermaid
sequenceDiagram
participant C as "客户端"
participant S as "sessionService"
participant R as "session 路由"
participant M as "sessionManager"
participant SC as "saveCover"
participant F as "文件系统"
C->>S : "uploadSessionImage()"
S->>R : "POST /api/session/ : sessionId/images"
R->>M : "saveInputImage(...)"
M->>F : "写入 input/<id><ext>"
R-->>S : "{ url }"
S-->>C : "持久化 URL"
C->>S : "putSessionState()"
S->>R : "PUT /api/session/ : sessionId/state"
R->>M : "saveState(...)"
M->>F : "写入 session.json"
Note over C,R : "欢迎页面：resolveSessionInfo() 同时返回预览URL和图像计数"
Note over C,R : "手动封面：setSessionCover() -> POST /api/session/ : sessionId/cover"
SC->>F : "复制图片到 cover{ext}"
SC->>M : "更新 session.json 标记 manualCover=true"
```

**图表来源**
- [client/src/services/sessionService.ts:69-113](file://client/src/services/sessionService.ts#L69-L113)
- [server/src/routes/session.ts:18-111](file://server/src/routes/session.ts#L18-L111)
- [server/src/services/sessionManager.ts:20-44](file://server/src/services/sessionManager.ts#L20-L44)
- [server/src/services/sessionManager.ts:91-110](file://server/src/services/sessionManager.ts#L91-L110)
- [server/src/services/sessionManager.ts:165-204](file://server/src/services/sessionManager.ts#L165-L204)

**章节来源**
- [client/src/services/sessionService.ts: API 定义:69-157](file://client/src/services/sessionService.ts#L69-L157)
- [server/src/routes/session.ts: 路由实现:18-111](file://server/src/routes/session.ts#L18-L111)

### 手动封面功能详解
- **新增** saveCover 函数实现
  - 参数验证：检查 sourceUrl 是否以 `/api/session-files/` 开头
  - 文件复制：从会话相对路径复制图片到 sessions/{sessionId}/cover{ext}
  - 扩展名处理：自动提取源文件扩展名，支持 .jpg/.png 等格式
  - 清理旧封面：删除会话目录中其他扩展名的 cover 文件
  - 状态标记：更新 session.json，设置 manualCover = true 和 coverExt
- **新增** SessionState 扩展
  - manualCover：布尔值，标记是否设置了手动封面
  - coverExt：字符串，存储封面图片的扩展名
- **新增** 欢迎页面封面逻辑
  - 优先检查 meta.manualCover 是否为 true
  - 如果有手动封面，直接使用 `/api/session-files/{sessionId}/cover{ext}`
  - 如果没有手动封面，按原有逻辑使用输出图或输入图作为预览
- **新增** API 使用方式
  - 客户端：setSessionCover(sessionId, sourceUrl)
  - 服务端：POST /api/session/:sessionId/cover，Body: { sourceUrl }

```mermaid
flowchart TD
Start(["设置会话封面"]) --> Validate["验证 sourceUrl 格式"]
Validate --> Valid{"格式有效?"}
Valid --> |否| Error["抛出错误"]
Valid --> |是| Extract["提取相对路径和扩展名"]
Extract --> Copy["复制文件到 cover{ext}"]
Copy --> Clean["清理其他扩展名的 cover 文件"]
Clean --> Mark["更新 session.json 标记 manualCover=true"]
Mark --> Return["返回 { coverUrl }"]
Error --> End(["结束"])
Return --> End
```

**图表来源**
- [server/src/services/sessionManager.ts:165-204](file://server/src/services/sessionManager.ts#L165-L204)
- [client/src/services/sessionService.ts:144-156](file://client/src/services/sessionService.ts#L144-L156)
- [client/src/components/WelcomePage.tsx:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)

**章节来源**
- [server/src/services/sessionManager.ts: 手动封面管理:165-204](file://server/src/services/sessionManager.ts#L165-L204)
- [client/src/services/sessionService.ts: 手动封面 API:144-156](file://client/src/services/sessionService.ts#L144-L156)
- [client/src/components/WelcomePage.tsx: 手动封面支持:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)

### per-tab 图像隔离与数据同步
- per-tab 隔离
  - 目录结构：每个 sessionId 下按 tab-0..9 维度组织 input 与 masks
  - 恢复时按 tab 重建 images，并根据 imageId 关联 tasks 与 selectedOutputIndex
- 数据同步
  - 图像：首次检测到新图像即异步上传，成功后更新 store 中的 sessionUrl
  - 遮罩：遮罩状态变化即保存 PNG，保存时解析 tabId（从 maskKey 解析 imageId，再在 store 中定位 tab）
  - 状态：工作流与遮罩状态分别订阅，仅序列化非文件字段写入 session.json
  - **新增** 封面：手动封面设置后不会影响其他数据同步
- 资源管理
  - 上传去重：uploadedImages/savedMasks 集合避免重复上传/保存
  - 空会话清理：欢迎页或新建会话时若为空且已保存过，删除服务器记录
  - **新增** 封面文件管理：自动清理不同扩展名的封面文件

```mermaid
flowchart TD
A["检测到新图像"] --> B["上传到 tab-n/input"]
B --> C["更新 store 中的 sessionUrl"]
D["遮罩状态变化"] --> E["生成 PNG Blob"]
E --> F["保存到 tab-n/masks/<safeName>.png"]
G["store 变更"] --> H["防抖序列化并保存 session.json"]
I["页面卸载"] --> J["sendBeacon 发送最终状态"]
K["欢迎页面统计"] --> L["计算输入图像 + 输出图像数量"]
M["设置手动封面"] --> N["复制图片到 cover{ext}"]
N --> O["更新 session.json 标记 manualCover=true"]
```

**图表来源**
- [client/src/hooks/useSession.ts:188-233](file://client/src/hooks/useSession.ts#L188-L233)
- [client/src/hooks/useSession.ts:235-265](file://client/src/hooks/useSession.ts#L235-L265)
- [client/src/hooks/useSession.ts:397-418](file://client/src/hooks/useSession.ts#L397-L418)
- [server/src/services/sessionManager.ts:165-204](file://server/src/services/sessionManager.ts#L165-L204)

**章节来源**
- [client/src/hooks/useSession.ts: 图像上传与去重:188-233](file://client/src/hooks/useSession.ts#L188-L233)
- [client/src/hooks/useSession.ts: 遮罩保存与 tab 解析:235-265](file://client/src/hooks/useSession.ts#L235-L265)
- [client/src/hooks/useSession.ts: 自动保存与卸载 flush:164-181](file://client/src/hooks/useSession.ts#L164-L181)
- [client/src/hooks/useSession.ts: 卸载前 flush:397-418](file://client/src/hooks/useSession.ts#L397-L418)

### 欢迎页面增强功能
- **新增** SessionCard 接口
  - meta：会话元数据（包含 sessionId、createdAt、updatedAt、**新增** manualCover、**新增** coverExt）
  - name：会话显示名称
  - previewUrl：会话预览图像 URL
  - **新增** imageCount：会话中图像总数（输入图像 + 输出图像）
- **重构** resolveSessionInfo 函数
  - **新增** 手动封面优先：当 meta.manualCover 为 true 时直接使用手动封面
  - 同时返回预览URL和图像计数
  - 统计所有标签页的输入图像和输出图像数量
  - 作为回退时使用第一个输入图像作为预览
- **UI 层增强**
  - 会话卡片底部显示图像数量和图标
  - 支持手动封面和自动封面两种模式
  - **新增** 封面状态指示：手动封面时显示不同的视觉标识
  - 提供更好的视觉反馈和用户体验

```mermaid
flowchart TD
Start(["欢迎页面加载"]) --> ListSessions["列出会话"]
ListSessions --> ResolveInfo["resolveSessionInfo()"]
ResolveInfo --> CheckManual{"manualCover?"}
CheckManual --> |是| UseManual["使用手动封面URL<br/>/api/session-files/{sessionId}/cover{ext}"]
CheckManual --> |否| LoadSession["加载会话数据"]
UseManual --> CountImages["统计图像数量"]
LoadSession --> CountImages
CountImages --> BuildCard["构建会话卡片<br/>显示预览 + 名称 + 时间 + 图像计数"]
BuildCard --> End(["完成"])
```

**图表来源**
- [client/src/components/WelcomePage.tsx:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
- [client/src/components/WelcomePage.tsx:13-18](file://client/src/components/WelcomePage.tsx#L13-L18)
- [client/src/components/WelcomePage.tsx:466-473](file://client/src/components/WelcomePage.tsx#L466-L473)

**章节来源**
- [client/src/components/WelcomePage.tsx: SessionCard 接口定义:13-18](file://client/src/components/WelcomePage.tsx#L13-L18)
- [client/src/components/WelcomePage.tsx: resolveSessionInfo 函数:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
- [client/src/components/WelcomePage.tsx: 图像计数显示:466-473](file://client/src/components/WelcomePage.tsx#L466-L473)

### 状态恢复算法详解
- 步骤
  - 读取 session.json，获取 activeTab 与 tabData
  - **新增** 检查 manualCover 标记
  - 如果 manualCover 为 true，直接使用 cover{ext} 作为封面
  - 对每个标签页：
    - 重建 images：拼接 /api/session-files/{sessionId}/tab-{n}/input/{id}{ext}，fetch → File → Blob URL
    - 探测 masks：按 maskKey 与安全名尝试 HEAD，存在则 fetch → RGBA 像素
  - 恢复工作流与遮罩状态：restoreSession 与 restoreAllMasks
  - 设置最后保存时间与停止恢复标志位
- 错误处理
  - 单个图像/遮罩失败不影响整体恢复，记录警告并跳过
  - 会话不存在返回空，按设置行为显示欢迎页或新建会话
  - **新增** 手动封面文件缺失：降级到自动封面或输入图作为回退

```mermaid
sequenceDiagram
participant Hook as "useSession"
participant API as "getSession()"
participant Svc as "sessionService"
participant R as "session 路由"
participant M as "sessionManager"
participant FS as "文件系统"
Hook->>API : "getSession(sessionId)"
API->>R : "GET /api/session/ : sessionId"
R->>M : "loadSession(...)"
M->>FS : "读取 session.json"
FS-->>M : "SessionState"
M-->>R : "SessionState"
R-->>API : "SessionState"
API-->>Hook : "SessionState"
Hook->>FS : "fetchAsFile()/fetchMaskEntry()"
Hook->>FS : "检查 manualCover 标记"
Hook->>WF : "restoreSession()"
Hook->>MSK : "restoreAllMasks()"
```

**图表来源**
- [client/src/services/sessionService.ts:115-121](file://client/src/services/sessionService.ts#L115-L121)
- [server/src/routes/session.ts:70-79](file://server/src/routes/session.ts#L70-L79)
- [server/src/services/sessionManager.ts:112-120](file://server/src/services/sessionManager.ts#L112-L120)
- [client/src/hooks/useSession.ts:316-366](file://client/src/hooks/useSession.ts#L316-L366)

**章节来源**
- [client/src/services/sessionService.ts: 获取会话:115-121](file://client/src/services/sessionService.ts#L115-L121)
- [server/src/routes/session.ts: 加载会话:70-79](file://server/src/routes/session.ts#L70-L79)
- [server/src/services/sessionManager.ts: 读取与解析:112-120](file://server/src/services/sessionManager.ts#L112-L120)
- [client/src/hooks/useSession.ts: 恢复逻辑:316-366](file://client/src/hooks/useSession.ts#L316-L366)

### 类型与数据模型
- 前端类型
  - ImageItem：包含 sessionUrl（持久化 URL）
  - TaskInfo：包含 outputs（文件名与 URL）
  - **更新** SessionCard：包含 meta、name、previewUrl、**新增** imageCount
  - **更新** SessionMeta：包含 sessionId、createdAt、updatedAt、**新增** manualCover、**新增** coverExt
  - Store 数据：TabData 包含 images、prompts、tasks、selectedOutputIndex、backPoseToggles、配置等
- 后端类型
  - SessionState、SerializedTabData、SerializedImage、SerializedTask
  - **更新** SessionMeta：包含 sessionId、createdAt、updatedAt、**新增** manualCover、**新增** coverExt
  - WebSocket 事件类型：Progress、Complete、Error

```mermaid
classDiagram
class SessionState {
+string sessionId
+string createdAt
+string updatedAt
+number activeTab
+Record<number, SerializedTabData> tabData
+boolean manualCover
+string coverExt
}
class SessionMeta {
+string sessionId
+string createdAt
+string updatedAt
+boolean manualCover
+string coverExt
}
class SessionCard {
+SessionMeta meta
+string name
+string previewUrl
+number imageCount
}
class SerializedTabData {
+SerializedImage[] images
+Record<string,string> prompts
+Record<string,SerializedTask> tasks
+Record<string,number> selectedOutputIndex
+Record<string,boolean> backPoseToggles
}
class SerializedImage {
+string id
+string originalName
+string ext
}
class SerializedTask {
+string promptId
+string status
+number progress
+Output[] outputs
+string error
}
SessionState --> SerializedTabData : "包含"
SessionCard --> SessionMeta : "包含"
SerializedTabData --> SerializedImage : "包含"
SerializedTabData --> SerializedTask : "包含"
```

**图表来源**
- [server/src/services/sessionManager.ts:61-89](file://server/src/services/sessionManager.ts#L61-L89)
- [client/src/types/index.ts:1-58](file://client/src/types/index.ts#L1-L58)
- [server/src/types/index.ts:10-36](file://server/src/types/index.ts#L10-L36)
- [client/src/components/WelcomePage.tsx:13-18](file://client/src/components/WelcomePage.tsx#L13-L18)

**章节来源**
- [server/src/services/sessionManager.ts: 类型定义:61-89](file://server/src/services/sessionManager.ts#L61-L89)
- [client/src/types/index.ts: 前端类型:1-58](file://client/src/types/index.ts#L1-L58)
- [server/src/types/index.ts: 事件类型:10-36](file://server/src/types/index.ts#L10-L36)
- [client/src/components/WelcomePage.tsx: SessionCard 类型:13-18](file://client/src/components/WelcomePage.tsx#L13-L18)

## 依赖关系分析
- 组件耦合
  - useSession 依赖 sessionService、useWorkflowStore、useMaskStore
  - sessionService 依赖后端路由
  - session 路由依赖 sessionManager
  - **新增** saveCover 路由依赖 sessionManager.saveCover
  - index.ts 注册路由并提供静态文件服务
  - **新增** WelcomePage 依赖 sessionService 和 sessionManager
- 外部依赖
  - multer：内存上传存储
  - express 静态服务：/api/session-files → sessions 目录
  - 浏览器：fetch、File、Blob、URL.createObjectURL、navigator.sendBeacon

```mermaid
graph LR
Hook["useSession.ts"] --> Svc["sessionService.ts"]
Svc --> Route["routes/session.ts"]
Route --> Mgr["services/sessionManager.ts"]
Route --> SaveCover["saveCover()"]
Mgr --> SaveCover
Index["index.ts"] --> Route
Hook --> WF["useWorkflowStore.ts"]
Hook --> MSK["useMaskStore.ts"]
WEL["WelcomePage.tsx"] --> Svc
WEL --> Mgr
```

**图表来源**
- [client/src/hooks/useSession.ts:1-16](file://client/src/hooks/useSession.ts#L1-L16)
- [client/src/services/sessionService.ts:1-157](file://client/src/services/sessionService.ts#L1-L157)
- [server/src/routes/session.ts:1-16](file://server/src/routes/session.ts#L1-L16)
- [server/src/services/sessionManager.ts:1-6](file://server/src/services/sessionManager.ts#L1-L6)
- [server/src/index.ts:53-60](file://server/src/index.ts#L53-L60)
- [client/src/components/WelcomePage.tsx:1-8](file://client/src/components/WelcomePage.tsx#L1-L8)

**章节来源**
- [server/src/index.ts: 路由注册与静态服务:53-60](file://server/src/index.ts#L53-L60)
- [server/src/routes/session.ts: 路由依赖:1-16](file://server/src/routes/session.ts#L1-L16)

## 性能考虑
- 上传与保存
  - 图像与遮罩采用异步上传，避免阻塞主线程
  - 防抖保存：500ms，减少频繁写入
- 文件系统
  - 输入图与遮罩按 tab 维度分隔，便于清理与并发访问
  - 遮罩安全命名避免 Windows 非法字符
  - **新增** 封面文件管理：自动清理不同扩展名的封面文件
- 网络
  - sendBeacon 在卸载前发送最终状态，保证数据不丢失
  - 列表与删除采用轻量 JSON，避免大文件传输
  - **新增** 手动封面复制：只复制图片文件，不传输大文件数据
- 存储
  - 输出文件不复制到会话目录，仅记录 URL，节省空间
  - 裁剪旧会话，默认保留最近 5 个
  - **新增** 封面文件清理：避免重复的封面文件占用空间
- **新增** 欢迎页面性能优化
  - 并行加载多个会话的预览和图像计数
  - 缓存会话数据，避免重复请求
  - 懒加载图像，提升首屏加载速度
  - **新增** 封面状态检查：手动封面时直接使用，无需额外请求

## 故障排除指南
- 会话恢复失败
  - 现象：部分图像或遮罩未恢复
  - 排查：检查 /api/session-files 下对应路径是否存在；确认 session.json 中的 id/ext 是否正确
  - 参考
    - [client/src/hooks/useSession.ts: 恢复流程:316-366](file://client/src/hooks/useSession.ts#L316-L366)
    - [sessions/187f40a5-66a0-4b2f-95fb-d6c9b6baeba6/session.json:1-492](file://sessions/187f40a5-66a0-4b2f-95fb-d6c9b6baeba6/session.json#L1-L492)
- 遮罩保存失败
  - 现象：遮罩未保存或文件名异常
  - 排查：确认 maskKey 中冒号被替换为下划线；检查 tabId 解析是否正确
  - 参考
    - [server/src/services/sessionManager.ts: 遮罩保存与安全名:46-57](file://server/src/services/sessionManager.ts#L46-L57)
    - [client/src/hooks/useSession.ts: 遮罩保存与 tab 解析:235-265](file://client/src/hooks/useSession.ts#L235-L265)
- 自动保存未生效
  - 现象：页面刷新后状态未持久化
  - 排查：确认 store 订阅是否触发；检查防抖定时器；确认 isRestoring 标志位
  - 参考
    - [client/src/hooks/useSession.ts: 自动保存与防抖:164-181](file://client/src/hooks/useSession.ts#L164-L181)
- 会话列表为空
  - 现象：/api/session 返回空数组
  - 排查：确认 sessions 目录存在；检查 session.json 是否损坏；确认静态服务映射
  - 参考
    - [server/src/index.ts: 静态服务与 sessions 目录:37-61](file://server/src/index.ts#L37-L61)
    - [server/src/services/sessionManager.ts: 列表与错误处理:130-148](file://server/src/services/sessionManager.ts#L130-L148)
- **新增** 手动封面设置失败
  - 现象：POST /api/session/:sessionId/cover 返回错误
  - 排查：确认 sourceUrl 格式正确（必须以 `/api/session-files/` 开头）；检查源文件是否存在；确认会话 ID 正确
  - 参考
    - [server/src/routes/session.ts: 手动封面端点:88-104](file://server/src/routes/session.ts#L88-L104)
    - [server/src/services/sessionManager.ts: saveCover 函数:165-204](file://server/src/services/sessionManager.ts#L165-L204)
- **新增** 手动封面显示异常
  - 现象：会话卡片显示的封面不是手动设置的图片
  - 排查：确认 session.json 中 manualCover 标记为 true；检查 cover{ext} 文件是否存在；确认扩展名正确
  - 参考
    - [client/src/components/WelcomePage.tsx: 手动封面支持:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
    - [server/src/services/sessionManager.ts: 手动封面管理:165-204](file://server/src/services/sessionManager.ts#L165-L204)
- **新增** 欢迎页面图像计数异常
  - 现象：会话卡片显示的图像数量不正确
  - 排查：确认 resolveSessionInfo 函数是否正确统计输入图像和输出图像；检查 session.json 格式是否完整
  - 参考
    - [client/src/components/WelcomePage.tsx: resolveSessionInfo 函数:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
    - [client/src/components/WelcomePage.tsx: 图像计数显示:466-473](file://client/src/components/WelcomePage.tsx#L466-L473)

**章节来源**
- [client/src/hooks/useSession.ts: 恢复与保存逻辑:316-418](file://client/src/hooks/useSession.ts#L316-L418)
- [server/src/services/sessionManager.ts: 列表与错误处理:130-148](file://server/src/services/sessionManager.ts#L130-L148)
- [server/src/index.ts: 静态服务与目录:37-61](file://server/src/index.ts#L37-L61)
- [client/src/components/WelcomePage.tsx: 欢迎页面功能:35-84](file://client/src/components/WelcomePage.tsx#L35-L84)
- [server/src/routes/session.ts: 手动封面端点:88-104](file://server/src/routes/session.ts#L88-L104)

## 结论
该会话管理系统通过清晰的 per-tab 隔离、事件驱动的自动保存与稳健的状态恢复机制，实现了跨页面的无缝会话延续。后端提供可靠的文件存储与清理策略，前端通过防抖与去重优化提升用户体验。**新增的手动封面图片功能进一步增强了会话管理能力，用户可以自由选择任意会话图片作为封面，系统通过自动封面再生保护机制确保手动设置的封面不会被后续操作覆盖。** 建议在生产环境启用定期备份与监控，确保数据安全与性能稳定。

## 附录

### API 使用示例（路径）
- 上传输入图
  - [client/src/services/sessionService.ts: uploadSessionImage:69-85](file://client/src/services/sessionService.ts#L69-L85)
- 上传遮罩
  - [client/src/services/sessionService.ts: uploadSessionMask:87-100](file://client/src/services/sessionService.ts#L87-L100)
- 保存会话状态
  - [client/src/services/sessionService.ts: putSessionState:102-113](file://client/src/services/sessionService.ts#L102-L113)
- 加载会话
  - [client/src/services/sessionService.ts: getSession:115-121](file://client/src/services/sessionService.ts#L115-L121)
- 列出会话
  - [client/src/services/sessionService.ts: listSessions:123-128](file://client/src/services/sessionService.ts#L123-L128)
- 删除会话
  - [client/src/services/sessionService.ts: deleteSession:129-133](file://client/src/services/sessionService.ts#L129-L133)
- **新增** 设置会话封面
  - [client/src/services/sessionService.ts: setSessionCover:144-156](file://client/src/services/sessionService.ts#L144-L156)
- **新增** 手动封面端点
  - [server/src/routes/session.ts: 手动封面端点:88-104](file://server/src/routes/session.ts#L88-L104)

### 最佳实践
- 数据备份
  - 定期导出会话目录（sessions/）与输出目录（output/）
  - 使用版本控制或云存储进行异地备份
- 性能优化
  - 控制单次上传文件大小，合理拆分批次
  - 使用防抖与去重策略，减少网络与磁盘压力
  - **新增** 欢迎页面采用并行加载和缓存策略提升性能
  - **新增** 手动封面复制采用文件系统直接复制，避免大文件传输
- 故障恢复
  - 会话损坏时优先修复 session.json；必要时删除并重新开始
  - 清理旧会话前确认不再需要，避免误删
  - **新增** 手动封面文件损坏时，系统会自动降级到自动封面或输入图
- **新增** 手动封面使用建议
  - 选择清晰、具有代表性的图片作为封面
  - 封面图片建议使用 .jpg 或 .png 格式，避免特殊字符
  - 设置手动封面后，系统会自动标记 manualCover=true，防止后续自动封面覆盖
  - 定期清理不需要的会话，保持会话列表整洁
  - 利用图像计数功能快速识别包含大量图像的会话
- **新增** 封面管理最佳实践
  - 封面文件会自动清理不同扩展名的旧文件，无需手动管理
  - 手动封面设置后，欢迎页面会优先显示手动封面
  - 如果需要恢复自动封面，可以删除手动封面文件并清除 session.json 中的标记