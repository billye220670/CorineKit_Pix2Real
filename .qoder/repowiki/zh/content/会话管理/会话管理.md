...
## 更新摘要
**变更内容**
- 新增WelcomePage组件的图像计数功能，增强会话卡片显示能力
- SessionCard接口新增imageCount字段，提供更丰富的会话信息
- resolveSessionInfo函数重构为同时返回预览URL和图像计数
- UI层新增图像计数显示，提升用户体验和会话管理效率
...
**更新** 本次更新增强了WelcomePage组件的功能，新增图像计数显示能力，为用户提供更直观的会话管理体验。
...
- 欢迎页面（前端）
  - **新增** 图像计数功能：统计会话中的输入图像和输出图像总数
  - **新增** 增强的会话卡片：显示会话名称、更新时间、预览图像和图像计数
- 入口与静态服务（后端）
...
ResolveInfo --> ManualCover{"手动封面?"}
ManualCover --> |是| UseCover["使用手动封面URL"]
ManualCover --> |否| LoadSession["加载会话数据"]
UseCover --> CountImages["统计图像数量"]
...
## 结论
该会话管理系统通过清晰的 per-tab 隔离、事件驱动的自动保存与稳健的状态恢复机制，实现了跨页面的无缝会话延续。后端提供可靠的文件存储与清理策略，前端通过防抖与去重优化提升用户体验。**新增的欢迎页面图像计数功能进一步增强了会话管理能力，为用户提供了更直观的会话概览和更好的使用体验。** 建议在生产环境启用定期备份与监控，确保数据安全与性能稳定。
