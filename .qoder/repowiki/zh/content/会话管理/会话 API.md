...
- 状态管理
  - saveState/loadSession：序列化/反序列化 SessionState，包含 createdAt/updatedAt
  - listSessions：列出所有会话并按 updatedAt 倒序排序
...
- typed API 封装：uploadSessionImage、uploadSessionMask、putSessionState、getSession、listSessions、deleteSession
...
## 结论
会话 API 通过清晰的分层设计与严格的类型约束，实现了可靠的会话创建、状态持久化、文件上传下载与自动化清理。客户端采用事件驱动与防抖策略，确保在复杂交互场景下的稳定性与性能。结合 WebSocket 的实时进度推送，整体用户体验流畅可靠。
