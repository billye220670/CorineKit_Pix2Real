...
import { buildWorkflow5Prompt, buildWorkflow7Prompt, buildWorkflow8Prompt, buildWorkflow9Prompt, buildWorkflow10Prompt } from '../services/workflowExec.js';
...
router.post('/5/execute', uploadFields, async (req, res) => {
  try {
    ...
    // Build prompt (shared builder)
    const template = buildWorkflow5Prompt(originalFilename, maskFilename, { backPose, prompt: userPrompt });
...
router.post('/10/execute', uploadFields, async (req, res) => {
  try {
    ...
    // Build prompt (shared builder)
    const template = buildWorkflow10Prompt(originalFilename, maskFilename, { backPose, prompt: userPrompt });
...
router.post('/7/execute', express.json(), async (req, res) => {
  try {
    ...
    if (referenceImage) {
      ...
      const proTemplate = buildWorkflow7Prompt({
        model, loras, prompt, negativePrompt, width, height, steps, cfg, sampler, scheduler, name, seed: clientSeed,
        referenceComfyName: comfyRefFilename,
        depthStrength: req.body.depthStrength,
        poseStrength: req.body.poseStrength,
        useOriginalRatio: req.body.useOriginalRatio,
      });
    ...
    const template = buildWorkflow7Prompt({
      model, loras, prompt, negativePrompt, width, height, steps, cfg, sampler, scheduler, name, seed: clientSeed,
    });
...
router.post('/9/execute', express.json(), async (req, res) => {
  try {
    ...
    const template = buildWorkflow9Prompt({
      unetModel, loras, shiftEnabled, shift, prompt, width, height, steps, cfg, sampler, scheduler, name,
    });
...
router.post('/8/execute', uploadFaceSwapFields, async (req, res) => {
  try {
    ...
    // Build prompt (shared builder)
    const template = buildWorkflow8Prompt(targetFilename, faceFilename);
